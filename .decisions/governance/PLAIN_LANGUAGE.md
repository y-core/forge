...
> Owns the **quality of prose** on two surfaces: the text inside `.decisions/` documents,
...
> obscure, or unusable.
>
> Defers to: [`AGENT_GUIDE.md`](./AGENT_GUIDE.md) for document *form* — frontmatter, numbering,
...
|---|---|---|
| Prose inside `.decisions/governance/` and `.decisions/implementation/` | **yes** | — |
...
and lengthens the path to it. The editing question is: *does the reader need this here, to do
what they came to do?* If not, delete it, move it, or make it supplementary.
...
The author's order, in a report of completed work: *what I explored → what I found → what I
changed → whether it passed.* The reader's order: *did it work → what changed → what I need to
decide → what I could not do.*
...
one. That block *is* the document's index — there is no other. A `## 0.` whose lines are
...
*before* over *prior to*, *if* over *in the event that*, *enough* over *a sufficient number of*.

**Simplification must not destroy precision.** A technical term is frequently the clearest term
available, and the reader identified in §3a already knows most of them. *Idempotent*, *isolate*,
*barrel*, *fail-closed* — each names something no paraphrase names as exactly. Keep the term.
Where it is genuinely novel to the reader, keep it *and explain it once*; do not replace it with
...
genuinely irrelevant, or deliberately secondary — *the connection was reset* is better than
...
Delete: repetition, empty openers (*it is worth noting that*, *as mentioned above*), hedging
...
stated rather than implied. *Because*, *however*, *therefore*, *before*, *after*, *if* — these
