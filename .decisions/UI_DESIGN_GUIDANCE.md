...
> browser tier and the SSR boundary; [`AGENT_GUIDE.md`](./AGENT_GUIDE.md) for the governing-doc
...
to close, and it is not a hypothetical one. An agent handed only the API reference can wire each
component correctly and still ship low-craft output:

- a raw hex literal where a semantic token is defined and themed,
- spacing chosen ad hoc, so no two gaps read as related or as deliberately different,
- four competing text colors on one surface,
- a `Card` nested inside a `Card`, producing two elevations that mean nothing,
- an eyebrow above every heading, including the ones that carry no section,
- no empty state on a collection that is empty on first load,
- a `Spinner` in a slot where a `Skeleton` matches the shape being awaited.

Each of those passes a type check, passes a test, and reads as a defect. The corpus's job is to
name them.
...
Consequences, and they are the same ones [`AGENT_GUIDE.md`](./AGENT_GUIDE.md) §2c draws for
section numbers:
...
corpus that teaches a component that no longer exists is worse than one that teaches nothing,
because it is followed.

The contract is one sentence: **the corpus may not describe an API forge does not have.**

Enforcement is a gate step. `scripts/validate-design.ts` owns the policy — what is asserted, what
fails, in what order, with what message — and `scripts/design-parse.ts` owns the matchers it
...
Default remains rebuttable in the sense §2b defines: `scripts/design-parse.ts` gives every rule a
...
The first row is unchanged from [`AGENT_GUIDE.md`](./AGENT_GUIDE.md) §6c — it is restated here
...
`scripts/validate-docs.ts` scopes to `.decisions/`, `CLAUDE.md`, the root `README.md`, every
...
- **Facts are usable.** Principles and numeric constants are not ownable, and the corpus states
  them plainly where they anchor a rule: the 45–75ch comfortable measure, the 4.5:1 and 3:1
  contrast ratios, spacing-scale steps that differ by at least 25% so no two gaps read as a
  mistake, 8–10 greys, roughly 5 elevation levels, 2 font weights, 2–3 text colors on a surface.
