...
export const TOOLS: readonly ToolSpec[] = [
  {
    ...
    description:
      "Rank the governing corpus against a question. Returns chunk ids of the form `canon/libs:CODE_RULES.md#5c` — pass one to knowledge_read. Search before inferring any architectural rule.",
    inputSchema: {
      type: "object",
      properties: {
        query: { ...STRING, description: "The question, in the words you would ask a colleague" },
        corpus: { ...STRING, enum: ["canon", "local"], description: "`canon` for the fleet's law, `local` for this repository's own" },
        tree: { ...STRING, enum: ["shared", "libs", "apps"], description: "Narrow to one canon tree" },
        path: { ...STRING, description: "A path prefix, to scope to one directory or document" },
  ...
  },
  {
    name: "knowledge_read",
    description:
      "Read one section whole, by the chunk id knowledge_search returned. `neighbours` also returns the sections either side, which is usually where the rule that scopes it lives.",
    inputSchema: {
      type: "object",
      properties: {
        id: { ...STRING, description: "A chunk id, e.g. `canon/libs:CODE_RULES.md#5c`" },
  ...
  switch (name) {
    case "knowledge_search": {
      ...
      const hits = search(knowledge.db, query, {
        ...
        ...(typeof args.tree === "string" ? { tree: args.tree } : {}),
        ...(typeof args.path === "string" ? { path: args.path } : {}),
        limit: typeof args.limit === "number" ? args.limit : 10,
      });
      if (hits.length === 0) return text(`${advisory}No match for "${query}".`);
      return text(advisory + hits.map((hit) => `${hit.id}\n  ${hit.headingPath}${hit.gloss === "" ? "" : `\n  ${hit.gloss}`}`).join("\n\n"));
    }
    ...
    case "knowledge_read": {
      ...
      return text(
        advisory + sections.map((section) => `## ${section.section}. ${section.title}   (${section.id})\n\n${section.body}`).join("\n\n---\n\n"),
      );
    }
