...
`ok` boolean. The repository's own `docs/ERROR_HANDLING.md` §1a publishes its exact
...
`undefined` remains correct for a value that is _legitimately optional_, where absence is not a
...
- A response that will be **swapped into an existing page** returns a _fragment_ — partial HTML
  with no document wrapper — so the surrounding page survives.
- A response that **is** the navigation returns a _full page_, with the document wrapper and the
...
restates none of it, and owns only what is specific to the _error path_ itself.
...
The distinction is depth. An error thrown _inside_ the middleware chain unwinds through the
chain, so the response still reaches whatever queues security headers. An error thrown _outside_
...
`docs/ERROR_HANDLING.md` §5b states the precedence rule among queued headers and the
...
repository's own `docs/ERROR_HANDLING.md` §5b names the depths and the guarantee each
one gives.
...
([`BOUNDARIES.md`](./BOUNDARIES.md) §4). What it _sends_ is governed by §3b.
...
in the owning `docs/` doc, not a licence to widen the rule.
