...
targets _configuration holders_ — objects that would otherwise expose partially-initialised
...
exception is recorded in the repository's `docs/` docs and in
...
_request-scoped_ data and its rationale is isolate recycling; a module loaded only by the browser
...
and it is paid for out of every future reader's attention.** This rule is a _ceiling_, not a
...
compiler or tooling _input_ that happens to use comment syntax: `@jsx*` pragmas, `biome-ignore`,
...
**3. A rare inline comment carrying a genuinely non-obvious _why_.** This is the exceptional
...
- **Justifying a decision the code cannot observe.** If two orderings, spellings, or shapes are
  ...
  genuinely matters, a _test_ asserts it — a comment cannot.
...
It is the _placement_ that is wrong. Route it to the one place that owns it:
...
the symbol itself. A question arrives as words — _where is the retry delay computed_ — and the
only index from those words to the code is the words already in the code.

This is the division of labour `CLAUDE.md` _Code Intelligence_ states, seen from the other end:
...
called, `*Options` says a bag of knobs is being passed. Neither says _what of_. A name assembled
...
hand it. Distinct from §3, which governs _untrusted_ input arriving at a boundary: §7d governs
_internal_ signatures, where the values are already trusted and the whole risk is positional.
