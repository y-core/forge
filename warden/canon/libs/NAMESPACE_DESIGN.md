...
> namespace. The repository's own catalog of namespaces lives in `docs/`.
>
...
place to shorten an import path — those are what a barrel means inside an _application_, and the
...
harms still apply, and a barrel's job is to _state_ its surface. Name the types.
...
is exported _or_ explicitly sealed, and never merely by existing.
...
When one namespace imports another's barrel, any future import in _that_ barrel which reaches
...
barrel is the _published surface_, not a convenience; consuming your own published surface from
within is how a namespace ends up depending on its own export map.

### 2c. Granting an Exemption

An exemption is warranted only where the barrel _is_ the abstraction — a facade whose whole
...
namespace a primitive. The repository's `docs/` docs name the current exemptions; this
...
_why_ a classification holds, which is the part prose is better at.
...
- **The §3c exemption is target-only.** An edge _into_ a primitive is exempt; an edge _out of_
...
part of its parent namespace, and its imports are the _parent's_ edges.
...
the copies encode a _behaviour_ rather than a presentation, this argument does not apply and the
...
`docs/` doc; an undocumented one is a defect.
