...
> Owns the five boundaries that every namespace is judged against. Each is a rule about _where_
...
CSRF is the boundary case worth naming: it is a _transport_ guard by placement in the
...
the option defaults to _not trusting_. A default of trust is a silent spoofing surface in every
...
the owning `docs/` document and listed in [`CODE_REVIEW.md`](./CODE_REVIEW.md) §6 so
