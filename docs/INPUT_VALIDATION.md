...
([`BOUNDARIES.md`](../warden/canon/libs/BOUNDARIES.md) §4a): the env validator's throw reached the app
...
`ValidationResult<T>` is the standard return type for a service function that validates its own
input. [`ERROR_HANDLING.md`](./ERROR_HANDLING.md) §1c owns the type; it is exported from
...
and only the second one is worth stating.**
...
refuse whitespace-only input cannot — by the time it runs, an all-spaces submission and a
well-formed one would be indistinguishable. A normalization the schema cannot see makes the parsed
...
See [`BOUNDARIES.md`](../warden/canon/libs/BOUNDARIES.md) §3 for the validate-at-boundary rule and the
ordered validation steps. This document owns only the concrete surface each step calls.
