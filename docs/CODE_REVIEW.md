...
See [`CODE_REVIEW.md`](../warden/canon/libs/CODE_REVIEW.md) §1 for the review workflow, the green-baseline
...
| Runtime namespaces use only Web APIs | [`LIBRARY_ARCHITECTURE.md`](../warden/canon/libs/LIBRARY_ARCHITECTURE.md) §1d |
| `valibot` is never imported outside the facade | [`INPUT_VALIDATION.md`](./INPUT_VALIDATION.md) §1a |
| Security-critical paths fail closed | [`BOUNDARIES.md`](../warden/canon/libs/BOUNDARIES.md) §5 |
...
[`UI_CLASS_COMPOSITION.md`](./UI_CLASS_COMPOSITION.md) §1e. The one legitimate spelling that is not a
...
([`NAMESPACE_DESIGN.md`](../warden/canon/libs/NAMESPACE_DESIGN.md) §2c), so this should return nothing. **PCRE2
...
**Unbudgeted comment** ([`CODE_RULES.md`](../warden/canon/libs/CODE_RULES.md) §5a is the whole
budget; [`CODE_RULES.md`](../warden/canon/libs/CODE_RULES.md) §5b is what is deleted on sight)
...
- The second needs `-P`: its lookahead is unsupported by the default engine, which errors rather
  ...
  hit and keep the one sentence [`CODE_RULES.md`](../warden/canon/libs/CODE_RULES.md) §5a permits.
...
- The sixth is the run-length detector: three or more consecutive `//` lines.
  [`CODE_RULES.md`](../warden/canon/libs/CODE_RULES.md) §5a form 3 caps an inline _why_ at
...
the defect ([`BOUNDARIES.md`](../warden/canon/libs/BOUNDARIES.md) §5a).
...
See [`CODE_REVIEW.md`](../warden/canon/libs/CODE_REVIEW.md) §4 for severity calibration, and for the
...
See [`CODE_REVIEW.md`](../warden/canon/libs/CODE_REVIEW.md) §5 for the verification protocol every finding
...
| --- | --- |
| `new Forge<Env>()` in a test | `Forge` is exported from `src/app/mod.ts` with a public constructor. The no-bare-constructor rule targets _config holders_ — [`CODE_RULES.md`](../warden/canon/libs/CODE_RULES.md) §1d |
...
| `import { v } from "../validation/mod"` in forge source | One of the two sanctioned barrel exemptions — [`NAMESPACE_DESIGN.md`](../warden/canon/libs/NAMESPACE_DESIGN.md) §2c |
...
| `@public` / `@internal` on a TSDoc line | Machine-readable visibility markers, explicitly budgeted — [`CODE_RULES.md`](../warden/canon/libs/CODE_RULES.md) §5a |
| A one-line inline comment carrying an external _why_ | The third budgeted form, subject to the four conditions in [`CODE_RULES.md`](../warden/canon/libs/CODE_RULES.md) §5a |
| A one-line note on an adversarial test fixture | The one test-side addition to the budget — [`CODE_RULES.md`](../warden/canon/libs/CODE_RULES.md) §5d |
...
| `[--tone:var(--color-…)]` arbitrary-property classes, and `bg-(--tone)` reading them | The tone mechanism, not a stray arbitrary value: `toneVariants` sets the properties and one recipe per appearance reads them — [`UI_CLASS_COMPOSITION.md`](./UI_CLASS_COMPOSITION.md) §1e |
| `state-invalid` or `cursor-pointer` passed as a separate `cn` argument, outside the base literal | Not an untidy call: the narrower recipe's group is a subset of an earlier one's, so folding it into the literal deletes it — [`UI_CLASS_COMPOSITION.md`](./UI_CLASS_COMPOSITION.md) §1e |
...
| --- | --- | --- |
| `unicorn/consistent-function-scoping` | 132 | 120 are test helpers declared inside a `describe` — the co-location idiom [`TESTING.md`](../warden/canon/libs/TESTING.md) §2a mandates |
...
([`TESTING.md`](../warden/canon/libs/TESTING.md)).
