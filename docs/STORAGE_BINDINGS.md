...
>
> Defers to: [`BOUNDARIES.md`](../warden/canon/libs/BOUNDARIES.md) §5a and §5b for the fail-closed posture
> and the `required: false` asymmetry; [`STRUCTURED_LOGGING.md`](./STRUCTURED_LOGGING.md)
> §2d for the KV channel-selection pattern; `src/storage/README.md` for worked usage and every
> client signature.
...
`createD1Client` wraps a raw `D1Database` with a typed client, taking the binding directly from
`c.env`. Query logging is opt-in and off by default — a client that logged every statement would
put query text, and whatever a bound parameter carried, onto a channel with no PII posture of its
own.
...
`resolveD1Client` reads the binding out of the request context via a caller-supplied selector and
builds the typed client, so a binding reference is never threaded through the call graph.
...
[`CODE_RULES.md`](../warden/canon/libs/CODE_RULES.md) §1a.** That rule prohibits _request-scoped_
...
[`NAMESPACES.md`](./NAMESPACES.md) §3b for why, and for the manual barrel discipline
...
`createKVStore` wraps a raw `KVNamespace` with a typed, codec-aware store, and its generic flows
...
`createObjectStore` wraps an `ObjectStorageBackend` — **not a raw `R2Bucket`**. Adapt a bucket
with `r2Backend` first (§3d).
...
non-security-critical features only (§5b, [`BOUNDARIES.md`](../warden/canon/libs/BOUNDARIES.md) §5b).
...
resolver constrained to it.** Each namespace publishes one — `D1DatabaseLike`, `KVNamespaceLike`,
`R2BucketLike` and R2's object and list satellites — and each resolver is generic in its binding
...
in-memory stub implementing only what forge actually calls satisfies the contract, so it can be
handed to `r2Backend`, `createKVStore` or `createD1Client` directly — no `@cloudflare/workers-types`
value in a test, and no fake obliged to implement a whole platform interface to be accepted. A
contract widened to mirror the platform type would silently take that away.

**Where a platform brand genuinely forces a cast, localise it once inside the adapter — never in
a resolver or a consumer.**
...
**A security-critical binding fails closed when it is missing** — a CSRF token store, an auth
session store, or the database backing user data **throws** rather than serving
([`BOUNDARIES.md`](../warden/canon/libs/BOUNDARIES.md) §5a owns the posture). Use `validateBindings` (§4b) to guarantee those bindings exist before the worker
