...
> Ships raw TypeScript. No build step. Consumed via `@ycore/forge/{namespace}` export map.
...
| `@ycore/forge/router` | Declarative route config | `route`, `index`, `layout`, `prefix`, `applyRoutes` |
| `@ycore/forge/security` | Security middleware & headers | `defineSecurity`, `makeSecurityHeaders`, `NONCE`, `csrfProtection`, `originGuard`, `rateLimit` |
