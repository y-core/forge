...
// Every scanner takes a string, so every fixture below is written inline — no tmpdir, no disk, and
// no `main()`. The one exception is the corpus-path assertion at the end, which is *about* the
// filesystem: a rule table pointing at a page nobody wrote is the failure it exists to catch.

const FILE = "src/ui/core/fixture.tsx";
const ROOT = resolve(dirname(fileURLToPath(import.meta.url)), "..");

/** A source of one line, so a scanner's whole output can be stated. */
...
describe("findColorLiterals() — literals inside a class position", () => {
  ...
  it("reports a literal once, however many patterns see it", () => {
    // `bg-[#fff]` is both an arbitrary-value bracket and a quoted string in class position. Two
    // findings for one mistake would double every failure count in the gate's output.
...
describe("findColorLiterals() — the constructs it must not flag", () => {
  // Every line here is drawn from `src/ui/**`. A failure below fails forge's own source.
  //
  // The status-hue lines carry their `dark:` halves because that is how they are now written —
  // `Alert`, `Toast` and `Badge` gained dark counterparts, so a light-only `bg-red-50` no longer
  // appears anywhere in `src/ui/**`. What this suite pins is unchanged either way: `findColorLiterals`
  // looks for raw literals (`#hex`, `rgb()`, `oklch()`, arbitrary-value brackets), and a palette
  // utility is not one whichever variant prefixes it. Whether a *bare* palette utility is allowed is
  // a question for `forge-ui-color-theme-no-raw-utility` in the corpus, not for this parser.
...
describe("findRawThemeUtilities() — the constructs it must not flag", () => {
  // The first line was `badge.tsx`'s own `warning` entry, and this comment used to say that a
  // failure here fails forge's own source. It no longer does: `badge.tsx` reaches the status hues
  // through `--status-*` tokens now, and no file in `src/` writes a paired palette utility at all.
  //
  // The fixture stays, and is the more interesting half of the rule for it. This matcher exists to
  // permit a *correctly paired* utility while failing an unpaired one, and forge having stopped
  // writing the permitted form is exactly when that half stops being covered by accident. A
  // consuming app is still free to write it — the rule is theirs to satisfy too.
  const allowed: string[] = [
  ...
  it("requires the counterpart to match on utility and hue, not merely to be some dark class", () => {
    // One `dark:` class must never excuse a whole map: `bg-red-100` is answered by `dark:bg-red-900`
    // and not by the `dark:text-red-200` sitting beside it.
  ...
  it("scopes the pairing to the line — a pair split across two lines still reports", () => {
    // The documented price of the line-scoped trade, and the case `isSuppressed` exists to answer.
...
describe("findInlineStyles()", () => {
  ...
  it("leaves a hyphenated attribute merely ending in `style` alone", () => {
    // This is what the lookbehind claims, and all it claims.
  ...
  it("flags a bare `style =` binding — the matcher is textual and cannot see it is not an attribute", () => {
    // Asserted rather than filed as a defect: the check runs over `.tsx` only, where a top-level
    // `style` binding does not appear, and narrowing the pattern to attribute position would need
    // the AST the rule deliberately does not build. Recorded so the boundary is not rediscovered.
...
describe("findArbitraryValues() — the brackets it must not flag", () => {
  // The variant brackets name a selector, not a value; the constraint utilities take a bound with
  // no scale equivalent. Each is written in `src/ui/**` today.
  const allowed: string[] = [
  ...
  it("honours the JSX expression-container suppression the corpus's own components use", () => {
    // The shape at `src/ui/core/accordion.tsx:70`.
    const source = [
...
// ── Rule ids, markers and citations ───────────────────────────────────────────────────────────
...
describe("findCustomPropertyCitations()", () => {
  ...
  it("does not also emit a bare citation for the family's own prefix", () => {
    // `--palette` is declared nowhere, so a leaked bare citation would fail the corpus for writing
    // the family form the family form exists to permit.
...
describe("findSourceViolations()", () => {
  ...
  it("includes the theme check, sorted in beside the colour-literal one it does not overlap", () => {
    // `bg-[#fff]` is a literal and `bg-yellow-100` is a palette utility: two rules, one line, and
    // neither scanner sees the other's case.
...
// ── formatFinding() and the rule table ────────────────────────────────────────────────────────
...
describe("formatFinding()", () => {
  ...
  it("routes each rule to its own corpus path", () => {
    const finding: Finding = { file: "a.tsx", line: 1, ruleId: "forge-ui-viewport-units", detail: "d" };

    expect(formatFinding(finding)).toBe("a.tsx:1: forge-ui-viewport-units — d (src/ui/design/floor.md)");
  });
});
...
describe("RULE_CORPUS_PATH", () => {
  const ids: RuleId[] = [
    ...
    // Enforced by `validate-contrast.ts` rather than by a scanner in this file — it is a property of
    // the stylesheet's stop mapping, not of any `.tsx` line — but it prints through the same table.
