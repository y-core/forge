...
/** Runs all three passes and returns the exit code rather than exiting, so a test can import this
 *  module, call it, and read the verdict without the runner dying. Every constant above this line is
 *  inert data or a pure function — no pass touches disk until `main` is called. */
export function main(): number {
  ...
  // passes forever while a *new* directory goes unscanned — which is exactly how `contracts/` shipped
  // a class-string literal that no consumer's Tailwind build ever saw. Asking disk to justify itself
