...
import { applyJsoncEdits } from "./jsonc-edit";
...
describe("applyJsoncEdits() — inserting a new member", () => {
  ...
  it("does not double a trailing comma in a single-line object", () => {
    expect(apply(`{ "a": 1, }`, [{ path: ["b"], value: 2 }])).toBe(`{ "a": 1, "b": 2 }`);
  });

  it("leaves a single-line trailing-comma insertion parseable", () => {
    const out = apply(`{ "a": 1, }`, [{ path: ["b"], value: 2 }]);
    expect(JSON.parse(stripJsonc(out)) as unknown).toEqual({ a: 1, b: 2 });
  });

  it("preserves CRLF in an empty multi-line object", () => {
    const src = `{\r\n  "a": {\r\n  }\r\n}`;
