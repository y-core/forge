...
import { countComments, findMember, formatPath, parseJsoncTree, skipTrivia, stripJsonc } from "./jsonc";
...
describe("stripJsonc() — trailing commas are token-aware", () => {
  ...
  it("leaves `//` and `/* */` inside a string alone", () => {
    expect(JSON.parse(stripJsonc('{"u": "http://x", "b": "a/*b*/c"}')) as unknown).toEqual({ u: "http://x", b: "a/*b*/c" });
  });

  it("does not let an escaped quote end the string", () => {
    expect(JSON.parse(stripJsonc('{"a": "x\\"// y"}')) as unknown).toEqual({ a: 'x"// y' });
  });
