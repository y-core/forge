...
describe("findAppRoot() — deriving the app root from an install path", () => {
  it("returns the directory holding node_modules", () => {
    expect(findAppRoot("/srv/app/node_modules/@y-core/forge/src/tooling/cli/app-root.ts")).toBe("/srv/app");
  });

  it("takes the first node_modules, so a nested install still names the app", () => {
    expect(findAppRoot("/srv/app/node_modules/x/node_modules/@y-core/forge/src/tooling/cli/app-root.ts")).toBe("/srv/app");
  });

  it("resolves a pnpm store path to the app rather than to the store entry", () => {
    expect(findAppRoot("/srv/app/node_modules/.pnpm/@y-core+forge@0.0.84/node_modules/@y-core/forge/src/tooling/cli/app-root.ts")).toBe("/srv/app");
  });
  ...
  it("returns undefined when the path is not inside a node_modules at all", () => {
    expect(findAppRoot("/src/forge/src/tooling/cli/app-root.ts")).toBeUndefined();
  });

  it("does not match a directory that merely begins with the name", () => {
    expect(findAppRoot("/srv/node_modules_backup/app/src/tooling/cli/app-root.ts")).toBeUndefined();
  });

  it("returns the empty string for an install at the filesystem root, not undefined", () => {
    expect(findAppRoot("/node_modules/@y-core/forge/src/tooling/cli/app-root.ts")).toBe("");
  });
});
