...
describe("truncate()", () => {
  it("returns the input untouched when it fits", () => {
    expect(truncate("hello", 8)).toEqual({ text: "hello", width: 5, truncated: false, index: 5 });
  });

  it("returns the input untouched at exactly the limit", () => {
    expect(truncate("hello", 5)).toEqual({ text: "hello", width: 5, truncated: false, index: 5 });
  });

  it("cuts to the limit and marks the cut", () => {
    expect(truncate("hello world", 8)).toEqual({ text: "hello w…", width: 8, truncated: true, index: 7 });
  });

  it("honours a custom ellipsis", () => {
    expect(truncate("hello world", 8, { ellipsis: "..." })).toEqual({ text: "hello...", width: 8, truncated: true, index: 5 });
  });

  it("cuts silently when the ellipsis is wider than the column", () => {
    expect(truncate("hello", 2, { ellipsis: "..." })).toEqual({ text: "he", width: 2, truncated: true, index: 2 });
  });

  it("never splits a wide character across the boundary", () => {
    expect(truncate("古池や", 3, { ellipsis: "" })).toEqual({ text: "古", width: 2, truncated: true, index: 1 });
  });

  it("closes the style it cut into, so the colour does not run on", () => {
    expect(truncate(`${RED}hello world${DEFAULT_FG}`, 8)).toEqual({
      text: `${RED}hello w${RESET}…`,
      width: 8,
      truncated: true,
      index: RED.length + 7,
    });
