...
const CASES: readonly [string, CapabilityInput, ColorLevel][] = [
  ...
  ["a 256-color TERM on a terminal", { env: { TERM: "xterm-256color" }, isTTY: true }, 2],
  ["a 256-color TERM when piped, so a redirected stream stays clean", { env: { TERM: "xterm-256color" }, isTTY: false }, 0],
  ["COLORTERM=truecolor when piped", { env: { COLORTERM: "truecolor" }, isTTY: false }, 0],
  ["Windows Terminal when piped", { env: { WT_SESSION: "abc" }, isTTY: false }, 0],
  ["GitHub Actions, which is never a TTY, still gets truecolor", { env: { CI: "1", GITHUB_ACTIONS: "1" }, isTTY: false }, 3],
