...
function planSecret(entry: SecretEntry, kind: DevVarKind, remoteNames: ReadonlySet<string>, ctx: HandlerContext): Plan {
  ...
  if (kind === "rotatable") {
    ...
    // generates it; `--rotate` is only what it takes to replace a value that already exists.
...
function writeRow(write: Extract<Plan, { kind: "write" }>, type: ResourceType, ok: boolean, detail: string): SyncResult {
  ...
  return {
    ...
    local: true,
    // `remote` is presence, not success: a failed write of a secret the listing just
    // proved exists leaves the old value sitting on the remote.
    remote: ok || write.existed,
...
/** The note for a `.dev.vars` that yielded no keys, distinguishing an absent file from an empty one. */
...
/** Creates a secrets handler reading the `.dev.vars` beside the given config path. */
function createHandler(configPath: string, spec: SecretsHandlerSpec): ResourceHandler<SecretEntry> {
  ...
  return {
    ...
    async reconcile(entries, ctx): Promise<ReconcileResult<SecretEntry>> {
      // Concluded without a request, and therefore without needing credentials.
