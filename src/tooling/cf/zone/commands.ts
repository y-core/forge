...
interface PhasePlan {
  ...
  errorDetail?: string;
  /** Set once this phase's own PUT succeeded, which is what `updated` claims on its row. */
  written?: boolean;
}
...
/** Recursively sorts object keys, leaving array order intact, so a comparison is about values. @internal */
...
/** Which of `sync`'s outcomes this phase is in, read from this phase's own write. @internal */
export function phaseAction(plan: PhasePlan): SyncAction {
  if (plan.error !== undefined) return "error";
  if (!phaseInSync(plan)) return "drift";
  return plan.written === true ? "updated" : "in-sync";
}

/** Renders one phase as a report row. @internal */
function phaseRow(plan: PhasePlan, style: Colorize): TableRow {
  return {
    Phase: plan.phase,
    Rules: String(plan.desired.length),
    Action: styleAction(phaseAction(plan), style),
...
async function runSyncZone(flags: LooseFlags, ctx?: CliContext): Promise<void> {
  ...
  if (flags.json === true) {
    ...
    const section: TableSection = {
      ...
      rows: plans.map((plan) => phaseRow(plan, style)),
    ...
    // `Action` and `Detail` are prose and may wrap; `Phase` is an identifier and truncates.
...
/** Why a phase could not be read or written: a short row reason, and Cloudflare's own words below. @internal */
