...
function stepFor(value: string): { step: string; negative: boolean } | undefined {
  ...
  return STEPS.has(String(multiple)) ? { step: String(multiple), negative: pixels < 0 } : undefined;
}
  ...
  create(context) {
    ...
    return classLiteralVisitor((found) => {
      ...
      for (const match of found.text.matchAll(ARBITRARY)) {
        ...
        // Tailwind compiles `-mt-[-16px]` to a positive offset, so the suggestion's sign is the XOR
        // of the utility's `-` prefix and the value's own negativity — not the prefix alone.
