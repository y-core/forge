...
describe("platform-logical-spacing", () => {
  ...
  it("reports the whole arbitrary value, which `(` and `%` used to truncate", () => {
    expect(runRule(platformLogicalSpacing, attribute("class", literal("mr-[calc(100%-1rem)]")))).toEqual([
      detail("mr-[calc(100%-1rem)]", "me-[calc(100%-1rem)]"),
    ]);
  });

  it("reports a custom-property width in the `(--x)` spelling", () => {
    expect(runRule(platformLogicalSpacing, attribute("class", literal("border-r-(--w)")))).toEqual([detail("border-r-(--w)", "border-e-(--w)")]);
  });
