...
describe("color-token-only", () => {
  ...
  for (const root of ["shadow", "inset-shadow", "drop-shadow", "text-shadow"]) {
    it(`leaves \`${root}-(--x)\` alone — the bare form compiles to the shadow value, not a colour`, () => {
      expect(runRule(colorTokenOnly, attribute("class", literal(`${root}-(--shadow-lg)`)))).toEqual([]);
    });
  }
