...
function expandPattern(root: string, entry: PatternEntry): string[] {
  ...
  return collectFiles(root, staticDir, () => true).filter((file) => covers(entry, file));
}
...
function collectOwnedSourceFiles(ownerDir: string, barrelDirs: Set<string>, barrelFile: string, ownTargets: Set<string>): string[] {
  const nested = listDirectories(ownerDir)
    .map((name) => resolve(ownerDir, name))
    .filter((dir) => !barrelDirs.has(dir))
    .flatMap((dir) => collectOwnedSourceFiles(dir, barrelDirs, barrelFile, ownTargets));
  const own = listFiles(ownerDir, (name) => /\.tsx?$/.test(name) && !/\.test\.tsx?$/.test(name))
    .map((name) => resolve(ownerDir, name))
    .filter((full) => full !== barrelFile && !ownTargets.has(full));
  return [...own, ...nested];
}
...
export async function checkExports(config: ExportsCheckConfig): Promise<CheckResult> {
  ...
  for (const { dir, extension } of config.assetDirs ?? []) {
    ...
    for (const entry of listFiles(base, (name) => name.endsWith(extension))) {
  ...
  if (checked === 0 && findings.length === 0) return scannedNothing(`\`${packageName}\` resolved no export subpath`, "exports", "verified");
