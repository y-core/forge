import { oklabToLinearSrgb, srgbGamma } from "../../../ui/contracts/theme/color";
...
// Per-channel clipping, not CSS Color 4 chroma reduction: a browser clips, and a conformance
// measurement must match what is painted — `ui/contracts/theme` gamut-maps because it generates.
...
export function oklchToPaintedHex(l: number, c: number, h: number): string {
  ...
  const linear = oklabToLinearSrgb(l, c * Math.cos(hRad), c * Math.sin(hRad));
  const channel = (value: number): string =>
    Math.round(Math.min(1, Math.max(0, srgbGamma(Math.max(value, 0)))) * 255)
      .toString(16)
      .padStart(2, "0");
