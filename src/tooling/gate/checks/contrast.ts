...
export function parsePalette(css: string): Map<string, string> {
  ...
  for (const match of blankComments(css).matchAll(/(--color-[a-z0-9]+(?:-[a-z0-9]+)*)\s*:\s*([^;}]+)[;}]/g)) {
...
// A `.dark` block and a `:root` block both weigh 0-1-0 and both match the document element, so a
// consumer sheet imported after forge's beats one half of a pair and leaks the other mode in.
...
function checkNoDarkBlock(root: string, cssDir: string): Finding[] {
  ...
  const files = listFiles(resolve(root, cssDir), (name) => name.endsWith(".css"));

  for (const name of files) {
    const source = blankComments(readFileSync(resolve(root, cssDir, name), "utf-8"));
    for (const match of source.matchAll(/\.dark(?![\w-])[^;{}()]*\{/g)) {
      ...
      findings.push(
        fail(
          ...
          { file: `${cssDir}/${name}`, line: lineAt(source, match.index) },
        ),
      );
    }
  }
...
export function checkContrast(config: ContrastCheckConfig): CheckResult {
  ...
  if (pairs.length === 0) return scannedNothing("no audited pairs", "contrast", "measured");
  ...
  const schemes = listFiles(
    resolve(root, cssDir),
    (name) => name.startsWith("theme-") && name.endsWith(".css") && !config.tokenFiles.includes(`${cssDir}/${name}`),
  );
