...
// A namespaced key is a slot no Tailwind utility can reach: a state recipe's compiled signature is
...
export function deriveClassGroups(ds: DesignSystem, stateRecipes: readonly string[] = []): ClassGroupTable {
  ...
  for (let i = 0; i < staticRoots.length; i += 1) {
    const name = staticRoots[i] as string;
    const group = conditional.has(bare(name)) ? stateRecipeGroup(bare(name)) : staticSignatures[i];
    if (group !== undefined) claim(name, group);
  }
  for (const name of conditional) {
    if (!statics.has(name)) throw new Error(`\`${name}\` is declared a state recipe but the design system compiles no such static utility.`);
  }
