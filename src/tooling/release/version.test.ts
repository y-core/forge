...
describe("resolveVersion() — explicit version", () => {
  ...
  it("refuses an explicit version against a latest tag it cannot parse, rather than skipping the guard", () => {
    expect(() => resolveVersion({ explicit: "0.9.0", cwd: CWD, tagPrefix: PREFIX }, makeDeps({ getLatestTag: () => "v1.0.0-rc.1" }))).toThrow(
      "Cannot parse version from tag: v1.0.0-rc.1",
    );
  });

  it("refuses an explicit version not greater than the release tag a prerelease was sorted above", () => {
    expect(() => resolveVersion({ explicit: "0.9.0", cwd: CWD, tagPrefix: PREFIX }, makeDeps({ getLatestTag: () => "v1.0.0" }))).toThrow(
      "Version 0.9.0 is not greater than the current tag v1.0.0",
    );
  });
});
...
describe("resolveVersion() — auto bump", () => {
  ...
  it("bumps from the release tag, not the prerelease git sorted above it", () => {
    // `getLatestTag` filters the prerelease out, so what reaches here is the release itself.
    const result = resolveVersion(
      { cwd: CWD, tagPrefix: PREFIX },
      makeDeps({ getLatestTag: () => "v1.0.0", getCommitsSinceTag: () => ["abc1234 fix: something"] }),
    );
    expect(result).toEqual({ version: "1.0.1", reason: "auto-patch", previous: "v1.0.0", evidence: { commitCount: 1 } });
  });
