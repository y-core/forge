...
// Two rails on one page otherwise announce "toolbar, toolbar"; the name belongs to the config, which
// is the one place a rail is described.
describe("Toolbar — the rail is named from its config", () => {
  it("cannot be configured without a label, and renders the one it is given", async () => {
    // @ts-expect-error — `label` is required on a ToolbarDefinition.
    const unnamed: ToolbarDefinition = { groups: [] };
    void unnamed;

    const config: ToolbarDefinition = { label: "Scene tools", groups: [] };

    expect(attrOf(await render(<Toolbar config={config} icon={icon} />), "aria-label")).toBe("Scene tools");
  });
});
...
describe("Toolbar — popover title", () => {
  ...
  it("renders the title as a flex row (label + no action button) without titleAction", async () => {
    const config: ToolbarDefinition = {
      label: "Scene tools",
      groups: [{ items: [{ kind: "popover", icon: "layers", label: "Layers", content: <div /> }] }],
    };
    const out = await render(<Toolbar config={config} icon={icon} />);
    expect(out).toBe(
      '<div role="toolbar" aria-label="Scene tools" data-slot="toolbar" data-scope="toolbar" data-orientation="vertical" aria-orientation="vertical" class="group flex flex-col items-center"><div data-slot="toolbar-group" class="flex w-full flex-col items-center gap-0.5"><div data-slot="toolbar-popover" class="relative flex w-full flex-col items-center"><button type="button" data-slot="toolbar-button toolbar-trigger" class="state-busy state-disabled inline-flex items-center justify-center gap-2 rounded-field border-field font-medium whitespace-nowrap focus-ring motion-safe:transition-colors h-control-sm text-sm px-0 w-control-sm [--tone:var(--color-foreground)] [--tone-fg:var(--color-background)] [--tone-text:var(--color-foreground)] [--tone-soft:var(--color-muted)] [--tone-soft-fg:var(--color-foreground)] [--tone-soft-border:var(--color-border)] border-transparent bg-transparent [--focus-ring:var(--color-ring)] text-foreground hover:bg-accent hover:text-accent-foreground" data-toolbar-item="" command="toggle-popover" commandfor="toolbar-flyout-left-0" aria-controls="toolbar-flyout-left-0" aria-expanded="false" aria-haspopup="dialog" title="Layers" aria-label="Layers"><svg data-slot="icon" viewBox="0 0 24 24" class="h-5 w-5" aria-hidden="true"><use href="/sprite.svg#icon-layers"></use></svg></button><div id="toolbar-flyout-left-0" role="dialog" aria-labelledby="toolbar-flyout-left-0-title" data-slot="toolbar-flyout" popover="auto" data-side="left" class="min-w-52 rounded-box border border-border bg-popover p-2 pb-2.5 text-popover-foreground shadow-md"><div id="toolbar-flyout-left-0-title" data-slot="toolbar-flyout-title" class="px-0.5 pt-0.5 pb-1.5 text-xs font-semibold tracking-wider text-muted-foreground uppercase flex items-center justify-between gap-2"><span>Layers</span></div><div data-slot="toolbar-flyout-body" class="flex max-h-[60vh] flex-col items-stretch gap-3.5 overflow-y-auto px-0.5 pt-1 pb-0.5"><div></div></div></div></div></div></div>',
    );
  });
...
describe("Toolbar — native popover flyout", () => {
  ...
  it("renders the flyout as a native popover carrying the linking id and placement", async () => {
    ...
    expect(out).toBe(
      '<div role="toolbar" aria-label="Scene tools" data-slot="toolbar" data-scope="toolbar" data-orientation="vertical" aria-orientation="vertical" class="group flex flex-col items-center"><div data-slot="toolbar-group" class="flex w-full flex-col items-center gap-0.5"><div data-slot="toolbar-popover" class="relative flex w-full flex-col items-center"><button type="button" data-slot="toolbar-button toolbar-trigger" class="state-busy state-disabled inline-flex items-center justify-center gap-2 rounded-field border-field font-medium whitespace-nowrap focus-ring motion-safe:transition-colors h-control-sm text-sm px-0 w-control-sm [--tone:var(--color-foreground)] [--tone-fg:var(--color-background)] [--tone-text:var(--color-foreground)] [--tone-soft:var(--color-muted)] [--tone-soft-fg:var(--color-foreground)] [--tone-soft-border:var(--color-border)] border-transparent bg-transparent [--focus-ring:var(--color-ring)] text-foreground hover:bg-accent hover:text-accent-foreground" data-toolbar-item="" command="toggle-popover" commandfor="toolbar-flyout-right-0" aria-controls="toolbar-flyout-right-0" aria-expanded="false" aria-haspopup="dialog" title="Layers" aria-label="Layers"><svg data-slot="icon" viewBox="0 0 24 24" class="h-5 w-5" aria-hidden="true"><use href="/sprite.svg#icon-layers"></use></svg></button><div id="toolbar-flyout-right-0" role="dialog" aria-labelledby="toolbar-flyout-right-0-title" data-slot="toolbar-flyout" popover="auto" data-side="right" class="min-w-52 rounded-box border border-border bg-popover p-2 pb-2.5 text-popover-foreground shadow-md"><div id="toolbar-flyout-right-0-title" data-slot="toolbar-flyout-title" class="px-0.5 pt-0.5 pb-1.5 text-xs font-semibold tracking-wider text-muted-foreground uppercase flex items-center justify-between gap-2"><span>Layers</span></div><div data-slot="toolbar-flyout-body" class="flex max-h-[60vh] flex-col items-stretch gap-3.5 overflow-y-auto px-0.5 pt-1 pb-0.5"><div></div></div></div></div></div></div>',
    );
  });
  ...
  it("mints a distinct id for each popover so triggers and flyouts stay paired", async () => {
    ...
    const out = await render(<Toolbar config={config} icon={icon} />);
    expect(out).toBe(
      '<div role="toolbar" aria-label="Scene tools" data-slot="toolbar" data-scope="toolbar" data-orientation="vertical" aria-orientation="vertical" class="group flex flex-col items-center"><div data-slot="toolbar-group" class="flex w-full flex-col items-center gap-0.5"><div data-slot="toolbar-popover" class="relative flex w-full flex-col items-center"><button type="button" data-slot="toolbar-button toolbar-trigger" class="state-busy state-disabled inline-flex items-center justify-center gap-2 rounded-field border-field font-medium whitespace-nowrap focus-ring motion-safe:transition-colors h-control-sm text-sm px-0 w-control-sm [--tone:var(--color-foreground)] [--tone-fg:var(--color-background)] [--tone-text:var(--color-foreground)] [--tone-soft:var(--color-muted)] [--tone-soft-fg:var(--color-foreground)] [--tone-soft-border:var(--color-border)] border-transparent bg-transparent [--focus-ring:var(--color-ring)] text-foreground hover:bg-accent hover:text-accent-foreground" data-toolbar-item="" command="toggle-popover" commandfor="toolbar-flyout-left-0" aria-controls="toolbar-flyout-left-0" aria-expanded="false" aria-haspopup="dialog" title="A" aria-label="A"><svg data-slot="icon" viewBox="0 0 24 24" class="h-5 w-5" aria-hidden="true"><use href="/sprite.svg#icon-a"></use></svg></button><div id="toolbar-flyout-left-0" role="dialog" aria-labelledby="toolbar-flyout-left-0-title" data-slot="toolbar-flyout" popover="auto" data-side="left" class="min-w-52 rounded-box border border-border bg-popover p-2 pb-2.5 text-popover-foreground shadow-md"><div id="toolbar-flyout-left-0-title" data-slot="toolbar-flyout-title" class="px-0.5 pt-0.5 pb-1.5 text-xs font-semibold tracking-wider text-muted-foreground uppercase flex items-center justify-between gap-2"><span>A</span></div><div data-slot="toolbar-flyout-body" class="flex max-h-[60vh] flex-col items-stretch gap-3.5 overflow-y-auto px-0.5 pt-1 pb-0.5"><div></div></div></div></div><div data-slot="toolbar-popover" class="relative flex w-full flex-col items-center"><button type="button" data-slot="toolbar-button toolbar-trigger" class="state-busy state-disabled inline-flex items-center justify-center gap-2 rounded-field border-field font-medium whitespace-nowrap focus-ring motion-safe:transition-colors h-control-sm text-sm px-0 w-control-sm [--tone:var(--color-foreground)] [--tone-fg:var(--color-background)] [--tone-text:var(--color-foreground)] [--tone-soft:var(--color-muted)] [--tone-soft-fg:var(--color-foreground)] [--tone-soft-border:var(--color-border)] border-transparent bg-transparent [--focus-ring:var(--color-ring)] text-foreground hover:bg-accent hover:text-accent-foreground" data-toolbar-item="" command="toggle-popover" commandfor="toolbar-flyout-left-1" aria-controls="toolbar-flyout-left-1" aria-expanded="false" aria-haspopup="dialog" title="B" aria-label="B"><svg data-slot="icon" viewBox="0 0 24 24" class="h-5 w-5" aria-hidden="true"><use href="/sprite.svg#icon-b"></use></svg></button><div id="toolbar-flyout-left-1" role="dialog" aria-labelledby="toolbar-flyout-left-1-title" data-slot="toolbar-flyout" popover="auto" data-side="left" class="min-w-52 rounded-box border border-border bg-popover p-2 pb-2.5 text-popover-foreground shadow-md"><div id="toolbar-flyout-left-1-title" data-slot="toolbar-flyout-title" class="px-0.5 pt-0.5 pb-1.5 text-xs font-semibold tracking-wider text-muted-foreground uppercase flex items-center justify-between gap-2"><span>B</span></div><div data-slot="toolbar-flyout-body" class="flex max-h-[60vh] flex-col items-stretch gap-3.5 overflow-y-auto px-0.5 pt-1 pb-0.5"><div></div></div></div></div></div></div>',
    );
  });
  ...
  it("namespaces flyout ids by an explicit id prop and stamps that id on the nav", async () => {
    ...
    expect(out).toBe(
      '<div id="scene-rail" role="toolbar" aria-label="Scene tools" data-slot="toolbar" data-scope="toolbar" data-orientation="vertical" aria-orientation="vertical" class="group flex flex-col items-center"><div data-slot="toolbar-group" class="flex w-full flex-col items-center gap-0.5"><div data-slot="toolbar-popover" class="relative flex w-full flex-col items-center"><button type="button" data-slot="toolbar-button toolbar-trigger" class="state-busy state-disabled inline-flex items-center justify-center gap-2 rounded-field border-field font-medium whitespace-nowrap focus-ring motion-safe:transition-colors h-control-sm text-sm px-0 w-control-sm [--tone:var(--color-foreground)] [--tone-fg:var(--color-background)] [--tone-text:var(--color-foreground)] [--tone-soft:var(--color-muted)] [--tone-soft-fg:var(--color-foreground)] [--tone-soft-border:var(--color-border)] border-transparent bg-transparent [--focus-ring:var(--color-ring)] text-foreground hover:bg-accent hover:text-accent-foreground" data-toolbar-item="" command="toggle-popover" commandfor="toolbar-flyout-scene-rail-0" aria-controls="toolbar-flyout-scene-rail-0" aria-expanded="false" aria-haspopup="dialog" title="Layers" aria-label="Layers"><svg data-slot="icon" viewBox="0 0 24 24" class="h-5 w-5" aria-hidden="true"><use href="/sprite.svg#icon-layers"></use></svg></button><div id="toolbar-flyout-scene-rail-0" role="dialog" aria-labelledby="toolbar-flyout-scene-rail-0-title" data-slot="toolbar-flyout" popover="auto" data-side="left" class="min-w-52 rounded-box border border-border bg-popover p-2 pb-2.5 text-popover-foreground shadow-md"><div id="toolbar-flyout-scene-rail-0-title" data-slot="toolbar-flyout-title" class="px-0.5 pt-0.5 pb-1.5 text-xs font-semibold tracking-wider text-muted-foreground uppercase flex items-center justify-between gap-2"><span>Layers</span></div><div data-slot="toolbar-flyout-body" class="flex max-h-[60vh] flex-col items-stretch gap-3.5 overflow-y-auto px-0.5 pt-1 pb-0.5"><div></div></div></div></div></div></div>',
    );
  });
});

describe("Toolbar — flyout name", () => {
  it("points aria-labelledby at a title element that is really in the same output", async () => {
    const config: ToolbarDefinition = {
      label: "Scene tools",
      groups: [{ items: [{ kind: "popover", icon: "layers", label: "Groups", content: <div /> }] }],
    };
    const out = await render(<Toolbar config={config} icon={icon} />);
    const named = /<div id="[^"]*" role="dialog" aria-labelledby="([^"]*)"/.exec(out)?.[1] ?? "";

    expect(named).not.toBe("");
    expect(attrOf(out, "data-slot", `id="${named}"`)).toBe("toolbar-flyout-title");
    expect(out.split(`id="${named}"`)).toHaveLength(2);
  });
});
...
describe("Toolbar — action dispatch", () => {
  ...
  // it put a relationship in the markup that a reader and a validator both have to chase.
  it("emits neither half of the invoker pair when commandTarget is omitted", async () => {
    const config: ToolbarDefinition = {
      label: "Scene tools",
      groups: [{ items: [{ kind: "action", icon: "cursor", label: "Select", action: "select", dispatch: "command" }] }],
    };

    const out = await render(<Toolbar config={config} icon={icon} />);

    expect(out).not.toContain("commandfor");
...
describe("Toolbar — attribute precedence", () => {
  it("lets a caller's explicit data-orientation beat the one computed from placement", async () => {
    const out = await render(<Toolbar config={{ label: "Scene tools", groups: [] }} icon={icon} data-orientation='caller-wins' />);
    expect(out).toBe(
      `<div role="toolbar" aria-label="Scene tools" data-slot="toolbar" data-scope="${TOOLBAR_SCOPE}" data-orientation="caller-wins" aria-orientation="vertical" class="group flex flex-col items-center"></div>`,
    );
  });
});
