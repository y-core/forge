...
describe("Toolbar — action dispatch", () => {
  it("emits data-on-click for a scope-dispatched action (default)", async () => {
    ...
    const out = await render(<Toolbar config={config} icon={icon} />);
    expect(out).toBe(
      '<nav role="toolbar" data-slot="toolbar" data-scope="toolbar" data-orientation="vertical" aria-orientation="vertical" class="group flex flex-col items-center"><div data-slot="toolbar-group" class="flex flex-col items-center gap-0.5 w-full"><button type="button" data-slot="toolbar-button toolbar-action" class="inline-flex items-center justify-center rounded-lg font-medium motion-safe:transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 text-foreground hover:bg-accent size-9 p-0" data-toolbar-item="" title="Select" aria-label="Select" data-on-click="select" data-tool="select"><svg data-slot="icon" viewBox="0 0 24 24" class="w-5 h-5" aria-hidden="true"><use href="/sprite.svg#icon-cursor"></use></svg></button></div></nav>',
    );
  });
