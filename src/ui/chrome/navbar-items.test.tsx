...
describe("Navbar — links", () => {
  ...
  // The bar link and the menu row are the same destination one level apart, so a reader gets the
  // same "you are here" either way rather than only on the branch that happens to be at depth 0.
  it("marks a current link inside a menu, not only one on the bar", async () => {
    const config: NavDefinition = { sections: [{ items: [{ label: "File", items: [{ label: "Here", href: "here", current: true }] }] }] };

    const out = await render(<Navbar config={config} resolveHref={(k) => `/${k}`} icon={icon} />);

    expect([...out.matchAll(/<a role="menuitem"[^>]*>/g)].map((match) => match[0])).toEqual([
      `<a role="menuitem" data-slot="menu-link-item" class="${MENU_ITEM}" href="/here" aria-current="page" data-selected="">`,
    ]);
  });
});
...
describe("Navbar — menu id scoping", () => {
  ...
  it("namespaces each bar's menu ids by its own id, so two bars on a page never collide", async () => {
    ...
    expect(idLinks(out)).toEqual({
      ids: [
        "primary",
        "navbar-menu-primary-0-trigger",
        "navbar-menu-primary-0",
        "secondary",
        "navbar-menu-secondary-0-trigger",
        "navbar-menu-secondary-0",
      ],
  ...
  it("falls back to the placement when no id is given, so a top bar and a bottom bar stay disjoint", async () => {
    ...
    expect(idLinks(out)).toEqual({
      ids: ["navbar-menu-top-0-trigger", "navbar-menu-top-0", "navbar-menu-bottom-0-trigger", "navbar-menu-bottom-0"],
      commandfor: ["navbar-menu-top-0", "navbar-menu-bottom-0"],
    });
  });
  ...
  it("keeps every trigger of a single bar pointed at that bar's own popups", async () => {
    ...
    expect(idLinks(out)).toEqual({
      ids: ["main", "navbar-menu-main-0-trigger", "navbar-menu-main-0", "navbar-menu-main-1-trigger", "navbar-menu-main-1"],
  ...
  it("collides when two bars share a placement and neither is given an id", async () => {
    ...
    expect(idLinks(out)).toEqual({
      ids: ["navbar-menu-top-0-trigger", "navbar-menu-top-0", "navbar-menu-top-0-trigger", "navbar-menu-top-0"],
      commandfor: ["navbar-menu-top-0", "navbar-menu-top-0"],
    });
  });
  ...
  it("an id on either bar is enough to separate two same-placement bars", async () => {
    ...
    expect(idLinks(out)).toEqual({
      ids: ["navbar-menu-top-0-trigger", "navbar-menu-top-0", "secondary", "navbar-menu-secondary-0-trigger", "navbar-menu-secondary-0"],
...
describe("Navbar — auth filters", () => {
  ...
  const link = (html: string, key: string) => html.match(new RegExp(`<a href="/route/${key}".*?</a>`))?.[0] ?? "";
  const nestedPopup = (html: string) => html.match(/<div id="navbar-menu-top-1"[^>]*>/)?.[0] ?? "";
  const GATED_POPUP =
    '<div id="navbar-menu-top-1" role="menu" aria-labelledby="navbar-menu-top-1-trigger" data-slot="menu-popup" data-scope="menu" popover="auto" data-side="inline-end" data-align="start" class="z-50 min-w-40 rounded-box border border-border bg-popover p-1 text-popover-foreground shadow-md outline-none" data-filter="admin" hidden>';

  it("hides an item whose filter list is empty, since no token can ever match it", async () => {
    const config: NavDefinition = { sections: [{ items: [{ label: "Delete tenant", href: "danger", filters: [] }] }] };
    const out = await render(<Navbar config={config} resolveHref={id} icon={icon} activeFilters={["admin"]} />);
    expect(link(out, "danger")).toBe(`<a href="/route/danger" data-slot="navbar-link" class="${BAR_LINK}" data-filter="" hidden>Delete tenant</a>`);
  });

  it("leaves an item with no filters property visible", async () => {
    const config: NavDefinition = { sections: [{ items: [{ label: "Home", href: "home" }] }] };
    const out = await render(<Navbar config={config} resolveHref={id} icon={icon} activeFilters={[]} />);
    expect(link(out, "home")).toBe(`<a href="/route/home" data-slot="navbar-link" class="${BAR_LINK}">Home</a>`);
  });

  it("stamps the filter on a gated submenu's popup, not only its trigger", async () => {
    const config: NavDefinition = {
      sections: [{ items: [{ label: "Top", items: [{ label: "Admin", items: [{ label: "Users", href: "users" }], filters: ["admin"] }] }] }],
    };
    const out = await render(<Navbar config={config} resolveHref={id} icon={icon} activeFilters={["guest"]} />);
    expect(nestedPopup(out)).toBe(GATED_POPUP);
  });

  it("stamps the filter on a gated megamenu submenu's popup too", async () => {
    const config: NavDefinition = {
      sections: [
        {
          items: [
            { label: "Top", items: [{ label: "Products", groups: [{ heading: "G", group: [{ label: "l", href: "l" }] }], filters: ["admin"] }] },
          ],
        },
      ],
    };
...
describe("Navbar — groups", () => {
  ...
  it("shares one counter with the menus, so a group and a menu in one bar never collide", async () => {
    ...
    const out = await render(<Navbar config={config} resolveHref={id} icon={icon} />);
    expect(attrs(out, "id")).toEqual(["navbar-group-top-0", "navbar-menu-top-1-trigger", "navbar-menu-top-1", "navbar-group-top-2"]);
...
describe("Navbar — megamenu", () => {
  ...
  it("renders the columns as role=group blocks and never as a role=menu", async () => {
    ...
    expect(attrs(out, "role")).toEqual(["dialog", "group", "group", "group", "group"]);
