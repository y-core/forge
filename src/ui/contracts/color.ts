...
// Alpha is quantised to the byte the hex will carry *before* `V` is solved, so the emitted colour
// composites correctly at the alpha actually written rather than at the unrepresentable one.
...
/** Twelve `#rrggbbaa` overlays that composite over the scale's own step 1 to reproduce each solid step. @public */
