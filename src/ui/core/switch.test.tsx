...
describe("Switch", () => {
  ...
  it("wires field id and name from the descriptor", async () => {
    expect(await render(<Switch field={{ name: "grid" }} />)).toBe(
      '<label data-slot="switch" data-orientation="horizontal" data-label-position="after" class="inline-flex items-center gap-2"><input data-slot="switch-input" type="checkbox" role="switch" class="peer sr-only" id="field-grid" name="grid" aria-describedby="field-grid-description"><span data-slot="switch-track" aria-hidden="true" class="relative h-5 w-9 shrink-0 rounded-full bg-input transition-colors peer-checked:bg-primary peer-focus-visible:ring-2 peer-focus-visible:ring-ring peer-disabled:opacity-50"><span data-slot="switch-thumb" class="absolute left-0.5 top-0.5 size-4 rounded-full bg-background transition-transform peer-checked:translate-x-4"></span></span></label>',
    );
  });
  ...
  it("adds aria-invalid and aria-describedby when the field is invalid", async () => {
    expect(await render(<Switch field={{ name: "grid", invalid: true }} />)).toBe(
      '<label data-slot="switch" data-orientation="horizontal" data-label-position="after" class="inline-flex items-center gap-2"><input data-slot="switch-input" type="checkbox" role="switch" class="peer sr-only" id="field-grid" name="grid" aria-describedby="field-grid-description field-grid-error" aria-invalid="true"><span data-slot="switch-track" aria-hidden="true" class="relative h-5 w-9 shrink-0 rounded-full bg-input transition-colors peer-checked:bg-primary peer-focus-visible:ring-2 peer-focus-visible:ring-ring peer-disabled:opacity-50"><span data-slot="switch-thumb" class="absolute left-0.5 top-0.5 size-4 rounded-full bg-background transition-transform peer-checked:translate-x-4"></span></span></label>',
    );
  });
