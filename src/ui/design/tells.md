...
A headline set with `bg-gradient-to-r` plus `bg-clip-text` and a transparent
...
A saturated purple `--primary` nobody chose,
...
`bg-black`, or a hex literal for the same, as the dark-mode ground.
...
`backdrop-blur` with a translucent white or black fill on a card, a header, or
...
`shadow-[4px_4px_0_...]`, or the same shape hand-built from a
...
A Floor violation — `forge-ui-no-nested-card`. It appears because "group these
...
Three features, three tiers, three benefits — always three,
...
A small uppercase tracked line — "FEATURES", "HOW IT WORKS" —
...
`01`, `02`, `03` set large and faint beside each heading. It reads as
...
A Floor violation, stated in [`floor.md`](./floor.md). It persists because it is the obvious
...
Every gap on the surface the same value — `gap-4` between fields, between
...
`scrollbar-width`, a `::-webkit-scrollbar` rule, or a hand-built thumb over a
...
A Floor violation — `forge-ui-heading-order`. It arrives one way almost
...
Every step one notch from the last — `text-sm`, `text-base`, `text-lg`,
...
`text-justify` on a paragraph column. It reads as generated because it is a
...
`text-xs` on paragraphs, on button labels, on anything a reader must
...
`tracking-wide` or `tracking-wider` on a paragraph. It reads as
...
A Floor violation — `forge-ui-no-fabricated-data`. It happens because a
...
A Floor violation — `forge-ui-no-fabricated-data`. "John Doe",
...
A Floor violation — `forge-ui-no-fabricated-data`, and the most damaging
...
Latin filler left in a `Card.Description`, an `Alert.Description`,
...
A Floor violation — `forge-ui-real-icons`. It happens because an emoji is
...
Also `forge-ui-real-icons`. The path data is usually recalled rather
...
A centred spinner in a region whose shape is already
...
A `Dialog` for a two-field form, a detail view, an optional filter panel. It
...
A fade on the page, a stagger on the list, a scale on each card, a
