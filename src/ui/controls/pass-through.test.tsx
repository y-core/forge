...
describe("controls — arbitrary attribute pass-through", () => {
  ...
  it("Input forwards an arbitrary data-* attribute", async () => {
    ...
    expect(out).toBe(
      '<input data-slot="input" class="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus-visible:border-ring focus-visible:ring-2 focus-visible:ring-ring/20 focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50" data-test-hook="in" data-field="b">',
    );
  });
