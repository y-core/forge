...
| `Input`, `Textarea`, `Select`                                                                                   | `<input>` / `<textarea>` / `<select>` | Accept an optional `field` descriptor to wire `id` / `name` / `aria-*`. `Select` requires an `icon` prop (a `ForgeIcon<"chevron-down">`) and compounds `.Option`, `.OptGroup`. `Input` also takes `format` — see below.                                                                     |
