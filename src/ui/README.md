...
optional `data-action` / `data-challenge` / `data-appearance`) holding a hidden fallback alert. Place it **inside** the `<form>` so the token input Cloudflare
injects is submitted with it; nothing else is required, because the container stamps `TURNSTILE_SCOPE` and `resume()`
...
| `action`     | `string`                                      | —              | Scopes the minted token, and is what makes `verifyTurnstile({ expectedAction })` usable.                                                                                                                                                                                                   |
...
rather than sent tokenless, and the button is pressable again for a retry.
