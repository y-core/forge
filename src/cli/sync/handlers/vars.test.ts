...
describe("varsHandler.reconcile() — worker script", () => {
  ...
  it("issues no write at all, with or without --commit", async () => {
    // A var is pushed by `wrangler deploy`, which rewrites the worker's bindings from
    // the config. Writing one here would be undone by that deploy, so this handler
    // reads and reports only — the assertion is on the requests, not on the rows.
