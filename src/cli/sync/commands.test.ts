...
import { createSyncCommand, parseResources, pendingRows, printResults, resolveRotation, summariseCommit } from "./commands";
...
describe("createSyncCommand()", () => {
  ...
  it("says in its description that it is read-only without --commit", () => {
    expect(createSyncCommand().description).toMatch(/Read-only unless --commit/);
  });
});
...
describe("the command surface is exactly two modes", () => {
  it("offers --commit as the only switch between them", () => {
    ...
    const flags = createSyncCommand().flags as Record<string, unknown>;
  ...
  it("exposes no second command to keep in step with the first", async () => {
    ...
    expect(Object.keys(mod).filter((k) => k.startsWith("create"))).toEqual(["createSyncCommand"]);
  });
});
...
describe("--check and --json", () => {
  ...
  // Three independent guarantees keep `--json` parseable. The payload is data rather than
  // presentation, so no styler ever touches it; the handler selects PLAIN the moment `--json` is
  // set; and FORCE_COLOR=3 — the one setting that defeats isTTY, NO_COLOR and CI detection alike —
  // is what these assert against, because anything weaker would pass by accident.
