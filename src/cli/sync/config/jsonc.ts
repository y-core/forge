...
export function parseJsoncTree(src: string): Result<JsoncNode, JsoncParseError> {
  try {
    ...
    if (rest < src.length) {
      return { ok: false, error: { message: "trailing content after the root value", offset: rest } };
    }
    return { ok: true, data: node };
  } catch (e) {
    if (e instanceof ParseFailure) return { ok: false, error: { message: e.message, offset: e.offset } };
    throw e;
  }
}
