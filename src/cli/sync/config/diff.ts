...
/**
 * A general deep diff between the config as parsed and the config as the handlers
 * left it.
 *
 * Deliberately general rather than a hand-walk of the four arrays that currently
 * merge: a hand-walk can only notice the changes it was told to look for, so a
 * mutation the writer would silently drop passes unnoticed. Here anything that is
 * not a primitive-valued set becomes an `unsupported` entry, which the writer turns
 * into a refusal naming the path.
 */
