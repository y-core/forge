...
import type { JsonPath, Primitive } from "./diff";
import { formatPath } from "./diff";
import type { JsoncMember, JsoncNode } from "./jsonc";
import { parseJsoncTree, skipTrivia } from "./jsonc";
...
function err(message: string, path?: JsonPath): Result<never, JsoncEditError> {
...
function spliceInsertMembers(src: string, obj: JsoncNode & { kind: "object" }, members: { key: string; value: Primitive }[]): Splice[] {
  ...
  if (obj.members.length === 0) {
    ...
    const nl = src.slice(newlineIndex - 1, newlineIndex) === "\r" ? "\r\n" : "\n";
  ...
  // A newline at or past the closing brace means the object ends on this line, so
  // there is no line structure to preserve — `>=`, not `>`, since `obj.end` is the
  // offset just past `}` and the newline sits exactly there for `{ "a": 1 }\n`.
  const singleLine = nlIndex === -1 || nlIndex >= obj.end;

  if (singleLine) {
    // `{ "a": 1 }` stays on one line.
    return [{ start: afterComma, end: afterComma, text: `, ${inline}` }];
  }
...
function resolveEdit(root: JsoncNode, edit: JsoncEdit): Result<Resolved, JsoncEditError> {
  ...
  if (path.length === 0) return err("cannot replace the root value", path);

  let node = root;
  for (let depth = 0; depth < path.length; depth++) {
    const seg = path[depth];
    // Unreachable: `depth` is bounded by `path.length`, so every segment is present.
    if (seg === undefined) return err(`no segment at depth ${depth}`, path);
    const last = depth === path.length - 1;

    if (typeof seg === "number") {
      if (node.kind !== "array") return err(`expected an array at ${formatPath(path.slice(0, depth))}`, path);
      const element = node.elements[seg];
      if (element === undefined) {
        // Appending to an array would change its shape, not just a value in it.
        return err(`no element at index ${seg} — this writer only edits values that already have a slot`, path);
      }
    ...
    if (node.kind !== "object") return err(`expected an object at ${formatPath(path.slice(0, depth))}`, path);
    const member: JsoncMember | undefined = node.members.find((m) => m.key === seg);

    if (member === undefined) {
      if (!last) return err(`no such key: ${formatPath(path.slice(0, depth + 1))}`, path);
      return { ok: true, data: { kind: "insert", obj: node, key: seg, value } };
    }
  ...
  return err("unreachable path resolution", path);
}
...
function replaceValue(target: JsoncNode, value: Primitive, path: JsonPath): Result<Resolved, JsoncEditError> {
  if (target.kind === "object" || target.kind === "array") {
    ...
    return err(`refusing to overwrite an ${target.kind} at ${formatPath(path)} with a single value`, path);
  }
  return { ok: true, data: { kind: "splices", splices: [{ start: target.start, end: target.end, text: JSON.stringify(value) }] } };
}
...
export function applyJsoncEdits(src: string, edits: JsoncEdit[]): Result<string, JsoncEditError> {
  if (edits.length === 0) return { ok: true, data: src };

  const tree = parseJsoncTree(src);
  if (!tree.ok) return err(`could not parse the config for editing: ${tree.error.message} at offset ${tree.error.offset}`);

  const splices: Splice[] = [];
  // Insertions are collected per target object, keyed by its offset, and emitted
  // once per object below. Insertion order is preserved, so members appear in the
  ...
  for (const edit of edits) {
    if (typeof edit.value === "object" && edit.value !== null) {
      return err(`only primitive values can be written; ${formatPath(edit.path)} is not one`, edit.path);
    }
  ...
  for (let i = 1; i < ordered.length; i++) {
    ...
    if (cur.start < prev.end) {
      return err(`edits overlap in the source between offsets ${cur.start} and ${prev.end}`);
    }
  }
