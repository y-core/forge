...
/**
 * Wraps `input` in this chain's sequences.
 *
 * Two rewrites earn their keep here. Walking the chain outward and turning each ancestor's close
 * back into its open is what makes `red(green(x) + "y")` restore red rather than reset to the
 * terminal default. Closing every active code at a newline and reopening it after is what stops a
 * background colour bleeding to the right edge of the terminal, and what keeps a multi-line cell
 * from breaking the grid drawn around it.
 */
