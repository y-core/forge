...
import { ANSI_PATTERN, CSI, ESC, hasAnsi } from "./ansi";
...
// The sticky clone the scanner anchors at `lastIndex`. See `RE_ANSI`'s note on why the
// `g`-flagged original must never be used here.
const RE_ANSI_STICKY = new RegExp(ANSI_PATTERN, "y");

// One emoji: a flag pair, or a pictograph that is emoji-presentation by default or forced there
// by VS16, optionally skin-toned. Requiring the presentation is what keeps a text-default
// pictograph such as `©` at one column, where `emoji-regex-xs` — which forge cannot depend on —
// would otherwise have been consulted.
const EMOJI_ATOM =
...
// East-Asian wide and full-width blocks. Box drawing (U+2500–257F) sits below the first range and
// the horizontal ellipsis (U+2026) outside all of them, so both stay one column — the two special
// cases the reference calls out explicitly.
...
export function truncate(input: string, limit: number, options: TruncateOptions = {}): TruncateResult {
  ...
  if (full.width <= limit) return { text: input, width: full.width, truncated: false };
  ...
  return { text: `${head}${hasAnsi(head) ? RESET : ""}${ellipsis}`, width: cut.width + ellipsisWidth, truncated: true };
}
