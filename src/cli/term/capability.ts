...
/**
 * The colour level a stream supports, decided from `input` and nothing else.
 *
 * Pure by construction: the caller supplies the environment, the TTY state and the command line,
 * so the twenty-odd precedence rules below are testable with exact-match assertions instead of by
 * mutating `process.env` and hoping no other test observes it.
 *
 * Order: `FORCE_COLOR`, then the disabling signals, then the enabling flags, then `TERM=dumb`,
 * then CI, then the terminal's own advertisements, and finally the TTY check. `FORCE_COLOR` is
 * first because it is the one setting whose whole purpose is to defeat every check after it.
 * @public
 */
export function resolveColorLevel(input: CapabilityInput): ColorLevel {
  ...
  // `process.platform` and `node:os` for one heuristic. Windows Terminal advertises itself; the
  // older `conhost` falls through to the TTY check and gets level 1, which it does support.
