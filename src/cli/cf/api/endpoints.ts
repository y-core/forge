 ...
 *
 * **Zone read access is not ruleset access.** [P] probed live 2026-08-29 against a token that
 * returns a full `GET /zones/{zone_id}` for the same zone: both
 * `/rulesets/phases/{phase}/entrypoint` reads answered HTTP 403, code 10000
 * "Authentication error". So a token can be valid, active, and scoped to the right
 * zone, and still read nothing here — `Zone: Read` does not reach the Ruleset
 * Engine, and the failure carries no hint that a *different* permission is what is
 * missing.
 *
 * The permission that unlocks both phases at once is account-scoped:
 * **Account → Account Rulesets: Edit**. That it is account-level, on an endpoint
 * addressed entirely by zone, is the part nothing in the response suggests — and
 * the reason both phases fail together, when their per-phase zone permissions
 * differ and a missing one of those would fail only one.
 *
