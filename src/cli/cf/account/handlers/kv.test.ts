...
describe("kvHandler.reconcile() — the id is the identity", () => {
  it("matches by id even when the namespace is titled something else", async () => {
    // The duplicate-creation bug: a namespace provisioned outside this tool carries a
    // title that no prefix rule produces, so a title-only lookup missed it and
    // --commit created a second namespace beside the one the worker binds.
