...
describe("planRotation()", () => {
  ...
  it("accepts a marked key", () => {
    expect(planRotation(vars, ["SESSION_SECRET"])).toEqual({ ok: true, names: ["SESSION_SECRET"] });
  });

  it("refuses an unmarked key — a third-party credential cannot be re-obtained", () => {
    expect(planRotation(vars, ["STRIPE_KEY"])).toEqual({ ok: false, unknown: [], unmarked: ["STRIPE_KEY"] });
  });

  it("refuses a name that is not in the file at all", () => {
    expect(planRotation(vars, ["TYPO"])).toEqual({ ok: false, unknown: ["TYPO"], unmarked: [] });
  });
  ...
  it("keeps the two refusals apart, since they need different fixes", () => {
    ...
    expect(plan).toEqual({ ok: false, unknown: ["TYPO"], unmarked: ["STRIPE_KEY"] });
  });
});

describe("describeRefusal()", () => {
  it("names the missing key and the file", () => {
    const message = describeRefusal({ ok: false, unknown: ["TYPO"], unmarked: [] }, "/p/.dev.vars");
    expect(message).toBe("Not defined in /p/.dev.vars: TYPO");
  });

  it("tells the reader what marking a key means before they add one", () => {
    const message = describeRefusal({ ok: false, unknown: [], unmarked: ["STRIPE_KEY"] }, "/p/.dev.vars");
