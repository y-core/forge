 ...
 *
 * Composed as a single string so the alignment is the grid's problem. Held apart from the
 * description column, which is the only one that wraps.
 */
...
export function formatHelp(command: CommandBase, options: HelpOptions = {}): string {
  ...
  // `collectFlags`, not `command.flags`: a persistent flag declared on an ancestor is accepted by
  // the parser, so listing only the command's own would document a surface narrower than the one
  // that works.
