...
export async function execute(root: CommandBase, argv?: string[], io?: CliIO): Promise<void> {
  ...
  try {
    ...
    // Descent stopped on a word that is not a subcommand. When the command takes no arguments,
    // that word can only be a mistyped subcommand — saying so beats letting it fall through as a
    // positional and surface as `takes no arguments, got 1`. Both guards matter: a command with a
    // real argument list still takes its argument.
