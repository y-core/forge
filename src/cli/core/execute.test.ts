...
describe("a word that is not a subcommand", () => {
  function tree() {
    ...
    addCommand(root, createCommand({ name: "verify", description: "Run the gate" }));
    addCommand(root, createCommand({ name: "release", description: "Cut a release" }));
    return root;
  }

  it("names the near miss instead of reporting an unwanted argument", async () => {
    const { io, err, state } = makeIO();
    await run(tree(), ["verfy"], io);
    expect(err).toEqual(['Error: Unknown command "verfy" for "forge". Did you mean "verify"?']);
    expect(state.exitCode).toBe(1);
  });

  it("says the word is unknown even when nothing is close to it", async () => {
    const { io, err } = makeIO();
    await run(tree(), ["deploy"], io);
    expect(err).toEqual(['Error: Unknown command "deploy" for "forge".']);
  });

  // Both guards matter: `release` takes an optional bump argument, and a command that accepts
  // arguments must still accept them rather than have them read as mistyped subcommands.
  it("leaves a command that takes arguments alone", async () => {
    const seen: string[][] = [];
    const root = createCommand({ name: "forge" });
    const release = createCommand({ name: "release", args: { kind: "range", min: 0, max: 1 }, run: (args) => void seen.push(args) });
    addCommand(root, release);
    addCommand(root, createCommand({ name: "verify" }));
    const { io, err } = makeIO();
    await run(root, ["release", "minor"], io);
    expect(err).toEqual([]);
    expect(seen).toEqual([["minor"]]);
  });
});
