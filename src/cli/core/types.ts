 ...
 *
 * The `multiple` branch is tested **before** `default` and `required`, so a repeatable flag is a
 * `string[]` whether or not it also carries either. Note the inference trap: a table written
 * `{ type: "string" as const, multiple: true }` widens `multiple` to `boolean` and falls through
 * to `string | undefined`. Call sites must write `multiple: true as const`.
 */
...
/**
 * What a handler is told about the terminal it is writing to.
 *
 * Threaded as a third parameter rather than detected once into a module-level singleton, which
 * `CODE_RULES.md` §1 forbids and which could not express the two levels anyway: `forge verify >
 * log.txt` must still colour its progress on the attached stderr while the redirected stdout
 * stays clean. Additive in both directions — the parameter is optional, so a two-parameter handler
 * stays assignable *and* a two-argument call still typechecks; no existing call site is forced to
 * change. `execute` always supplies it. @public
 */
