...
export function formatSummary(input: SummaryInput, style: Colorize = PLAIN): string {
  ...
  if (input.failedAt !== undefined) {
    ...
    return `${style.red("✗")} ${input.gate} — failed at \`${input.failedAt}\` (${progress}, ${formatDuration(input.ms)})${scoped}`;
  }
  const plural = input.selected === 1 ? "step" : "steps";
  return `${style.green("✓")} ${input.gate} — ${input.selected} ${plural} passed (${formatDuration(input.ms)})${scoped}`;
}
...
export function formatMissingRequirement(label: string, tool: string, hint: string, style: Colorize = PLAIN): string {
  return `${style.red("✗")} ${label} — ${tool} not found; run \`${hint}\``;
}
