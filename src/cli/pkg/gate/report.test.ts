...
describe("formatFindingBlock()", () => {
  ...
  it("indents each finding to the depth a command step's failure excerpt uses", () => {
    expect(formatFindingBlock([fail("barrel omits `parseThing`", { file: "src/cli/pkg/mod.ts", line: 12 })])).toBe(
      "    FAIL src/cli/pkg/mod.ts:12: barrel omits `parseThing`",
    );
  });
...
describe("report colour", () => {
  ...
  it("carries the styler into the banner a summary appends", () => {
    expect(formatSummary({ gate: "verify", ran: 1, selected: 1, total: 15, ms: 100 }, style)).toBe(
