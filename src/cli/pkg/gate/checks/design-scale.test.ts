...
const CONFIG: DesignScaleCheckConfig = { root: ROOT, stylesheet: "src/ui/assets/css/tailwind.css", table: "src/cli/pkg/lint/data/design-scale.ts" };

// Asserted here rather than left to be discovered as a red gate: oxlint loads the plugin through
// Node's ESM resolver, so an import in the generated module would break every forge rule at once —
// and this assertion still signals on a machine with no `tailwindcss` installed.
...
describe("checkDesignScale()", () => {
  ...
  it.skipIf(!hasTailwind())("fails with a regeneration hint when the module is missing", async () => {
    const result = await checkDesignScale({ ...CONFIG, table: "src/cli/pkg/lint/data/absent.ts" });
