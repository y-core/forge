...
export function createGateCommand(config: GateCommandConfig): Command<typeof gateFlags> {
  ...
  return createCommand({
    ...
    async run(_args, flags, ctx) {
      ...
      for (const step of steps) {
        ...
        if (step.requires) {
          ...
          if (!(probe === undefined ? hasTool(tool) : probe())) {
            console.log(formatMissingRequirement(step.label, tool, hint, style));
            failedAt = step.label;
            break;
          }
        }
      ...
      console.log(
        formatSummary(
          { gate: banner, ran, selected: steps.length, total, ms: Date.now() - started, ...(failedAt !== undefined ? { failedAt } : {}) },
          style,
        ),
      );
