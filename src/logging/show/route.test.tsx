...
describe("loadLogViewer — HTMX load-more (cursor)", () => {
  it("returns the rows plus the out-of-band load-more row carrying the new cursor and the filters", async () => {
    ...
    expect(res.status).toBe(200);
    expect(await res.text()).toBe(
      `${ROW_HTML}<tr id="log-load-more" hx-swap-oob="outerHTML:#log-load-more"><td colspan="5" class="px-4 py-2 text-center"><button type="button" data-slot="button" class="inline-flex items-center justify-center rounded-lg font-medium whitespace-nowrap focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50 motion-safe:transition-colors text-foreground hover:bg-accent h-8 px-3 text-sm" hx-get="/admin/logs?cursor=cursor-2&amp;level=error&amp;q=pay+ment" hx-target="#log-tbody" hx-swap="beforeend" hx-indicator="this" hx-disabled-elt="this">Load more</button></td></tr>`,
    );
  });
  ...
  it("uses a custom basePath for both the detail and the next-page URLs", async () => {
    ...
    expect(await res.text()).toBe(
      `${ROW_HTML.replaceAll("/admin/logs?detail=", "/dashboard/logs?detail=")}<tr id="log-load-more" hx-swap-oob="outerHTML:#log-load-more"><td colspan="5" class="px-4 py-2 text-center"><button type="button" data-slot="button" class="inline-flex items-center justify-center rounded-lg font-medium whitespace-nowrap focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50 motion-safe:transition-colors text-foreground hover:bg-accent h-8 px-3 text-sm" hx-get="/dashboard/logs?cursor=cursor-2" hx-target="#log-tbody" hx-swap="beforeend" hx-indicator="this" hx-disabled-elt="this">Load more</button></td></tr>`,
    );
  });
  ...
  it("reports the stream complete for a filter submit, and incomplete with the cursor kept for an append", async () => {
    ...
    expect({ filterSubmit, append }).toEqual({
      filterSubmit:
        '<tbody id="log-tbody"><tr><td colspan="5" class="px-4 py-4"><div data-slot="alert" data-variant="destructive" class="relative rounded-2xl border py-3 ps-4 pe-4 text-sm border-status-danger-border bg-status-danger-subtle text-status-danger-subtle-foreground flex flex-col items-start gap-2"><div data-slot="alert-title" class="leading-none font-medium tracking-tight">Could not read the log stream</div><div data-slot="alert-description" class="text-sm leading-relaxed text-pretty opacity-90">The channel did not answer. Entries already loaded are still shown below.</div><button type="button" data-slot="button" class="inline-flex items-center justify-center rounded-lg font-medium whitespace-nowrap focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50 motion-safe:transition-colors border border-input text-foreground hover:bg-accent h-8 px-3 text-sm" hx-get="/admin/logs?level=error" hx-target="#log-tbody" hx-swap="outerHTML" hx-indicator="this" hx-disabled-elt="this">Retry</button></div></td></tr></tbody>',
      append:
        '<tr id="log-load-more" hx-swap-oob="outerHTML:#log-load-more"><td colspan="5" class="px-4 py-2 text-center"><div data-slot="alert" data-variant="destructive" class="relative grid gap-1.5 rounded-2xl border py-3 ps-4 pe-4 text-sm border-status-danger-border bg-status-danger-subtle text-status-danger-subtle-foreground mb-2 text-left"><div data-slot="alert-title" class="leading-none font-medium tracking-tight">Could not load the next page</div><div data-slot="alert-description" class="text-sm leading-relaxed text-pretty opacity-90">The channel did not answer. The entries already loaded are unaffected.</div></div><button type="button" data-slot="button" class="inline-flex items-center justify-center rounded-lg font-medium whitespace-nowrap focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50 motion-safe:transition-colors text-foreground hover:bg-accent h-8 px-3 text-sm" hx-get="/admin/logs?cursor=abc&amp;level=error" hx-target="#log-tbody" hx-swap="beforeend" hx-indicator="this" hx-disabled-elt="this">Try again</button></td></tr>',
    });
  });
});
