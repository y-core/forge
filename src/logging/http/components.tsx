...
export const LogFilterBar: FC<LogFilterBarProps> = ({ level, q, targetId, formAction, icon }) => (
  <form
    class="flex flex-wrap sm:flex-nowrap items-end gap-3"
    hx-get={formAction}
    hx-target={`#${targetId}`}
    hx-swap="outerHTML"
    hx-push-url="true"
  >
    ...
    <Field name="level" class="flex-1 max-w-xs">
      ...
      <Select name="level" field={{ name: "level" }} icon={icon}>
        <Select.Option value="" selected={!level}>All</Select.Option>
        <Select.Option value="debug" selected={level === "debug"}>debug</Select.Option>
        <Select.Option value="info" selected={level === "info"}>info</Select.Option>
        <Select.Option value="warn" selected={level === "warn"}>warn</Select.Option>
        <Select.Option value="error" selected={level === "error"}>error</Select.Option>
      </Select>
    </Field>
...
export const LogViewerContent: FC<{ data: LogViewerLoaderData; icon: ForgeIcon<"chevron-down"> }> = ({
  data,
  icon,
}) => (
