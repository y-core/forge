...
const ROW_B_HTML =
  '<tr class="border-b border-border hover:bg-accent"><td class="py-2 ps-4 pe-4 font-mono text-xs whitespace-nowrap text-muted-foreground tabular-nums">2026-05-31T10:00:01.000Z</td><td class="py-2 pe-4"><span data-slot="badge" data-tone="destructive" data-appearance="soft" class="inline-flex items-center rounded-selector border-field font-medium px-2.5 py-0.5 text-xs [--tone:var(--color-destructive)] [--tone-fg:var(--color-destructive-foreground)] [--tone-text:var(--color-destructive-text)] [--tone-soft:var(--color-status-danger-subtle)] [--tone-soft-fg:var(--color-status-danger-subtle-foreground)] [--tone-soft-border:var(--color-status-danger-border)] border-(--tone-soft-border) bg-(--tone-soft) text-(--tone-soft-fg) [--focus-ring:var(--color-ring)] hover:bg-[color-mix(in_oklab,var(--tone-soft),var(--tone)_8%)]">error</span></td><td class="py-2 pe-4 font-mono text-xs text-muted-foreground">api</td><td class="max-w-xs truncate py-2 pe-4 text-foreground"><button type="button" data-slot="button" class="state-busy state-disabled inline-flex items-center justify-center gap-2 rounded-field border-field font-medium whitespace-nowrap focus-ring motion-safe:transition-colors h-control-sm px-3 text-sm [--tone:var(--color-foreground)] [--tone-fg:var(--color-background)] [--tone-text:var(--color-foreground)] [--tone-soft:var(--color-muted)] [--tone-soft-fg:var(--color-foreground)] [--tone-soft-border:var(--color-border)] border-transparent bg-transparent [--focus-ring:var(--color-ring)] text-foreground hover:bg-accent hover:text-accent-foreground" aria-expanded="false" aria-controls="log-detail-logs--2026-05-31T10-00-01-000Z--bbb" hx-get="/admin/logs?detail=logs%7C%7C2026-05-31T10%3A00%3A01.000Z%7C%7Cbbb" hx-target="#log-detail-logs--2026-05-31T10-00-01-000Z--bbb" hx-swap="outerHTML" hx-indicator="#log-detail-logs--2026-05-31T10-00-01-000Z--bbb" hx-disabled-elt="this">Tom &amp; Co &lt;script&gt;</button></td><td class="py-2 pe-4 font-mono text-xs text-muted-foreground">req-7</td></tr><tr id="log-detail-logs--2026-05-31T10-00-01-000Z--bbb" class="hidden border-b border-border [&amp;.htmx-request]:table-row"><td colspan="5" class="px-4 py-2"><span class="sr-only">Loading log entry detail…</span><div data-slot="skeleton" aria-hidden="true" class="rounded-field bg-muted motion-safe:animate-pulse h-4 w-full"></div></td></tr>';

const ERROR_ROW_HTML =
  '<tr><td colspan="5" class="px-4 py-4"><div data-slot="alert" data-tone="destructive" data-appearance="soft" class="relative rounded-box border-field py-3 ps-4 pe-4 text-sm [--tone:var(--color-destructive)] [--tone-fg:var(--color-destructive-foreground)] [--tone-text:var(--color-destructive-text)] [--tone-soft:var(--color-status-danger-subtle)] [--tone-soft-fg:var(--color-status-danger-subtle-foreground)] [--tone-soft-border:var(--color-status-danger-border)] border-(--tone-soft-border) bg-(--tone-soft) text-(--tone-soft-fg) [--focus-ring:var(--color-ring)] hover:bg-[color-mix(in_oklab,var(--tone-soft),var(--tone)_8%)] flex flex-col items-start gap-2" data-announce-failure="Could not read the log stream. The channel did not answer. Entries already loaded are still shown below."><div data-slot="alert-title" class="leading-none font-medium tracking-tight">Could not read the log stream</div><div data-slot="alert-description" class="text-sm leading-relaxed text-pretty opacity-90">The channel did not answer. Entries already loaded are still shown below.</div><button type="button" data-slot="button" class="state-busy state-disabled inline-flex items-center justify-center gap-2 rounded-field border-field font-medium whitespace-nowrap focus-ring motion-safe:transition-colors h-control-sm px-3 text-sm [--tone:var(--color-foreground)] [--tone-fg:var(--color-background)] [--tone-text:var(--color-foreground)] [--tone-soft:var(--color-muted)] [--tone-soft-fg:var(--color-foreground)] [--tone-soft-border:var(--color-border)] bg-transparent [--focus-ring:var(--color-ring)] border-input text-foreground hover:bg-accent hover:text-accent-foreground" hx-get="/admin/logs" hx-target="#log-tbody" hx-swap="outerHTML" hx-indicator="this" hx-disabled-elt="this">Retry</button></div></td></tr>';
...
const EMPTY_FILTERED_ROW_HTML =
  '<tr><td colspan="5" class="px-4 py-4 text-center"><div class="flex flex-col items-center gap-2"><span class="text-sm text-muted-foreground">No log entries match these filters — the stream itself may not be empty.</span><button type="button" data-slot="button" class="state-busy state-disabled inline-flex items-center justify-center gap-2 rounded-field border-field font-medium whitespace-nowrap focus-ring motion-safe:transition-colors h-control-sm px-3 text-sm [--tone:var(--color-foreground)] [--tone-fg:var(--color-background)] [--tone-text:var(--color-foreground)] [--tone-soft:var(--color-muted)] [--tone-soft-fg:var(--color-foreground)] [--tone-soft-border:var(--color-border)] bg-transparent [--focus-ring:var(--color-ring)] border-input text-foreground hover:bg-accent hover:text-accent-foreground" hx-get="/admin/logs" hx-target="#log-tbody" hx-swap="outerHTML" hx-indicator="this" hx-disabled-elt="this" hx-push-url="/admin/logs">Clear filters</button></div></td></tr>';

const EMPTY_FILTERED_MORE_ROW_HTML =
  '<tr><td colspan="5" class="px-4 py-4 text-center"><div class="flex flex-col items-center gap-2"><span class="text-sm text-muted-foreground">No log entries match these filters on this page. Load more to keep searching.</span><button type="button" data-slot="button" class="state-busy state-disabled inline-flex items-center justify-center gap-2 rounded-field border-field font-medium whitespace-nowrap focus-ring motion-safe:transition-colors h-control-sm px-3 text-sm [--tone:var(--color-foreground)] [--tone-fg:var(--color-background)] [--tone-text:var(--color-foreground)] [--tone-soft:var(--color-muted)] [--tone-soft-fg:var(--color-foreground)] [--tone-soft-border:var(--color-border)] bg-transparent [--focus-ring:var(--color-ring)] border-input text-foreground hover:bg-accent hover:text-accent-foreground" hx-get="/admin/logs" hx-target="#log-tbody" hx-swap="outerHTML" hx-indicator="this" hx-disabled-elt="this" hx-push-url="/admin/logs">Clear filters</button></div></td></tr>';
...
describe("LogLevelBadge", () => {
  it("renders error as the destructive badge", async () => {
    expect(await render(<LogLevelBadge level='error' />)).toBe(
      '<span data-slot="badge" data-tone="destructive" data-appearance="soft" class="inline-flex items-center rounded-selector border-field font-medium px-2.5 py-0.5 text-xs [--tone:var(--color-destructive)] [--tone-fg:var(--color-destructive-foreground)] [--tone-text:var(--color-destructive-text)] [--tone-soft:var(--color-status-danger-subtle)] [--tone-soft-fg:var(--color-status-danger-subtle-foreground)] [--tone-soft-border:var(--color-status-danger-border)] border-(--tone-soft-border) bg-(--tone-soft) text-(--tone-soft-fg) [--focus-ring:var(--color-ring)] hover:bg-[color-mix(in_oklab,var(--tone-soft),var(--tone)_8%)]">error</span>',
    );
  });

  it("renders warn as the warning badge", async () => {
    expect(await render(<LogLevelBadge level='warn' />)).toBe(
      '<span data-slot="badge" data-tone="warning" data-appearance="soft" class="inline-flex items-center rounded-selector border-field font-medium px-2.5 py-0.5 text-xs [--tone:var(--color-warning)] [--tone-fg:var(--color-warning-foreground)] [--tone-text:var(--color-warning-text)] [--tone-soft:var(--color-status-warning-subtle)] [--tone-soft-fg:var(--color-status-warning-subtle-foreground)] [--tone-soft-border:var(--color-status-warning-border)] border-(--tone-soft-border) bg-(--tone-soft) text-(--tone-soft-fg) [--focus-ring:var(--color-ring)] hover:bg-[color-mix(in_oklab,var(--tone-soft),var(--tone)_8%)]">warn</span>',
    );
  });

  it("renders info as the info badge", async () => {
    expect(await render(<LogLevelBadge level='info' />)).toBe(
      '<span data-slot="badge" data-tone="info" data-appearance="soft" class="inline-flex items-center rounded-selector border-field font-medium px-2.5 py-0.5 text-xs [--tone:var(--color-info)] [--tone-fg:var(--color-info-foreground)] [--tone-text:var(--color-info-text)] [--tone-soft:var(--color-status-info-subtle)] [--tone-soft-fg:var(--color-status-info-subtle-foreground)] [--tone-soft-border:var(--color-status-info-border)] border-(--tone-soft-border) bg-(--tone-soft) text-(--tone-soft-fg) [--focus-ring:var(--color-ring)] hover:bg-[color-mix(in_oklab,var(--tone-soft),var(--tone)_8%)]">info</span>',
    );
  });
  ...
  it("renders debug as the outline badge — a neutral label, not a status hue", async () => {
    expect(await render(<LogLevelBadge level='debug' />)).toBe(
      '<span data-slot="badge" data-tone="neutral" data-appearance="outline" class="inline-flex items-center rounded-selector border-field font-medium px-2.5 py-0.5 text-xs [--tone:var(--color-foreground)] [--tone-fg:var(--color-background)] [--tone-text:var(--color-foreground)] [--tone-soft:var(--color-muted)] [--tone-soft-fg:var(--color-foreground)] [--tone-soft-border:var(--color-border)] border-(--tone-text) bg-transparent text-(--tone-text) [--focus-ring:var(--color-ring)] hover:bg-(--tone-soft)">debug</span>',
    );
  });
});
...
describe("LogFilterBar", () => {
  ...
  it("pre-selects the level and escapes the search value it echoes back", async () => {
    expect(await render(<LogFilterBar level='error' q="Tom & O'Brien" targetId={LOG_TBODY_ID} formAction='/admin/logs' icon={icon} />)).toBe(
      '<form class="flex flex-wrap items-end gap-2 sm:flex-nowrap" hx-get="/admin/logs" hx-target="#log-tbody" hx-swap="outerHTML" hx-indicator="#log-tbody" hx-disabled-elt="find button[type=&#39;submit&#39;]" hx-push-url="true"><fieldset data-slot="field" data-orientation="vertical" class="group/field flex w-full gap-3 data-[invalid]:text-destructive-text flex-col [&amp;&gt;*]:w-full min-w-xs flex-1"><label data-slot="field-label" class="flex w-fit items-center gap-2 text-sm leading-snug font-medium text-foreground group-data-[disabled]/field:opacity-50" for="field-q">Search</label><input data-slot="input" data-size="md" class="state-busy state-disabled state-invalid field-chrome focus-ring h-control-md text-sm" name="q" type="search" placeholder="message, prefix, requestId…" value="Tom &amp; O&#39;Brien" id="field-q"></fieldset><fieldset data-slot="field" data-orientation="vertical" class="group/field flex w-full gap-3 data-[invalid]:text-destructive-text flex-col [&amp;&gt;*]:w-full max-w-xs flex-1"><label data-slot="field-label" class="flex w-fit items-center gap-2 text-sm leading-snug font-medium text-foreground group-data-[disabled]/field:opacity-50" for="field-level">Level</label><div data-slot="select-wrapper" class="group/select relative w-full has-[select:disabled]:opacity-50"><select data-slot="select" data-size="md" class="state-busy state-disabled state-invalid field-chrome appearance-none pe-10 focus-ring h-control-md text-sm" name="level" id="field-level"><option data-slot="select-option" value="">All</option><option data-slot="select-option" value="debug">debug</option><option data-slot="select-option" value="info">info</option><option data-slot="select-option" value="warn">warn</option><option data-slot="select-option" value="error" selected>error</option></select><span aria-hidden="true" data-slot="select-icon" class="pointer-events-none absolute inset-y-0 end-3 flex items-center text-muted-foreground"><svg data-slot="icon" width="16" height="16" viewBox="0 0 16 16" class="" aria-hidden="true" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><use href="/sprite.svg#icon-chevron-down"></use></svg></span></div></fieldset><button type="submit" data-slot="button" class="state-busy state-disabled inline-flex items-center justify-center gap-2 rounded-field border-field font-medium whitespace-nowrap focus-ring motion-safe:transition-colors h-control-sm px-3 text-sm [--tone:var(--color-primary)] [--tone-fg:var(--color-primary-foreground)] [--tone-text:var(--color-primary-text)] [--tone-soft:var(--color-primary-soft)] [--tone-soft-fg:var(--color-primary-soft-foreground)] [--tone-soft-border:var(--color-primary-soft-border)] border-transparent bg-(--tone) text-(--tone-fg) [--focus-ring:var(--tone-fg)] hover:bg-[color-mix(in_oklab,var(--tone),var(--color-background)_12%)]">Filter</button></form>',
    );
  });
});
...
describe("LogLoadMoreRow", () => {
  it("renders the control appending into the tbody it sits outside of", async () => {
    expect(await render(<LogLoadMoreRow cursor='c1' complete={false} loadMoreAction='/admin/logs' />)).toBe(
      '<tr id="log-load-more"><td colspan="5" class="px-4 py-2 text-center"><button type="button" data-slot="button" class="state-busy state-disabled inline-flex items-center justify-center gap-2 rounded-field border-field font-medium whitespace-nowrap focus-ring motion-safe:transition-colors h-control-sm px-3 text-sm [--tone:var(--color-foreground)] [--tone-fg:var(--color-background)] [--tone-text:var(--color-foreground)] [--tone-soft:var(--color-muted)] [--tone-soft-fg:var(--color-foreground)] [--tone-soft-border:var(--color-border)] border-transparent bg-transparent [--focus-ring:var(--color-ring)] text-foreground hover:bg-accent hover:text-accent-foreground" hx-get="/admin/logs?cursor=c1" hx-target="#log-tbody" hx-swap="beforeend" hx-indicator="this" hx-disabled-elt="this">Load more</button></td></tr>',
    );
  });
  ...
  it("becomes its own retry when the read failed — the Alert above it, and Try again on the same cursor", async () => {
    expect(await render(<LogLoadMoreRow cursor='c1' complete={false} loadMoreAction='/admin/logs' failed={true} />)).toBe(
      '<tr id="log-load-more"><td colspan="5" class="px-4 py-2 text-center"><div data-slot="alert" data-tone="destructive" data-appearance="soft" class="relative grid gap-1.5 rounded-box border-field py-3 ps-4 pe-4 text-sm [--tone:var(--color-destructive)] [--tone-fg:var(--color-destructive-foreground)] [--tone-text:var(--color-destructive-text)] [--tone-soft:var(--color-status-danger-subtle)] [--tone-soft-fg:var(--color-status-danger-subtle-foreground)] [--tone-soft-border:var(--color-status-danger-border)] border-(--tone-soft-border) bg-(--tone-soft) text-(--tone-soft-fg) [--focus-ring:var(--color-ring)] hover:bg-[color-mix(in_oklab,var(--tone-soft),var(--tone)_8%)] mb-2 text-start" data-announce-failure="Could not load the next page. The channel did not answer. The entries already loaded are unaffected."><div data-slot="alert-title" class="leading-none font-medium tracking-tight">Could not load the next page</div><div data-slot="alert-description" class="text-sm leading-relaxed text-pretty opacity-90">The channel did not answer. The entries already loaded are unaffected.</div></div><button type="button" data-slot="button" class="state-busy state-disabled inline-flex items-center justify-center gap-2 rounded-field border-field font-medium whitespace-nowrap focus-ring motion-safe:transition-colors h-control-sm px-3 text-sm [--tone:var(--color-foreground)] [--tone-fg:var(--color-background)] [--tone-text:var(--color-foreground)] [--tone-soft:var(--color-muted)] [--tone-soft-fg:var(--color-foreground)] [--tone-soft-border:var(--color-border)] border-transparent bg-transparent [--focus-ring:var(--color-ring)] text-foreground hover:bg-accent hover:text-accent-foreground" hx-get="/admin/logs?cursor=c1" hx-target="#log-tbody" hx-swap="beforeend" hx-indicator="this" hx-disabled-elt="this">Try again</button></td></tr>',
    );
  });
  ...
  it("carries the active filters into the next-page URL, so page two is drawn from the same set", async () => {
    expect(await render(<LogLoadMoreRow cursor='c1' complete={false} loadMoreAction='/admin/logs' level='error' q='pay ment' />)).toBe(
      '<tr id="log-load-more"><td colspan="5" class="px-4 py-2 text-center"><button type="button" data-slot="button" class="state-busy state-disabled inline-flex items-center justify-center gap-2 rounded-field border-field font-medium whitespace-nowrap focus-ring motion-safe:transition-colors h-control-sm px-3 text-sm [--tone:var(--color-foreground)] [--tone-fg:var(--color-background)] [--tone-text:var(--color-foreground)] [--tone-soft:var(--color-muted)] [--tone-soft-fg:var(--color-foreground)] [--tone-soft-border:var(--color-border)] border-transparent bg-transparent [--focus-ring:var(--color-ring)] text-foreground hover:bg-accent hover:text-accent-foreground" hx-get="/admin/logs?cursor=c1&amp;level=error&amp;q=pay+ment" hx-target="#log-tbody" hx-swap="beforeend" hx-indicator="this" hx-disabled-elt="this">Load more</button></td></tr>',
    );
  });
});
...
describe("LogTableBody — empty states", () => {
  ...
  it("says the filters matched nothing on this page when a further page exists", async () => {
    expect(await render(<LogTableBody rows={[]} loadMoreAction='/admin/logs' level='error' more={true} />)).toBe(
      `<tbody>${EMPTY_FILTERED_MORE_ROW_HTML}</tbody>`,
    );
  });

  it("keeps the unfiltered wording even when a further page exists", async () => {
    expect(await render(<LogTableBody rows={[]} loadMoreAction='/admin/logs' more={true} />)).toBe(`<tbody>${EMPTY_UNFILTERED_ROW_HTML}</tbody>`);
  });
});
