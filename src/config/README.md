...
1. **The binding/env surface** (`KVNamespace`s, `R2Bucket`s, secrets, vars) is owned by `wrangler.jsonc` — so its schema should be **generated**, not hand-written. `forge gen-env` (`@y-core/forge/cli/cfgen`) emits `env.schema.ts` from `wrangler.jsonc` + `.dev.vars`, and `validateBindings(EnvSchema)` enforces it on the first request. A missing or malformed binding then fails loudly at the edge, never silently downstream.
...
Wire the generator as a script — `"gen:env": "forge gen-env"` — and re-run it whenever `wrangler.jsonc` bindings change. Full generator reference: [src/validation/README.md](../validation/README.md) (`@y-core/forge/cli/cfgen`); `validateBindings`/`validateEnv` reference: [src/app/README.md](../app/README.md).
