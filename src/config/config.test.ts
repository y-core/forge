...
describe("applyMapping", () => {
  ...
  it("maps a flat object shape", () => {
    const result = applyMapping({ DB_URL: "postgres://localhost", PORT: "5432" }, {
      dbUrl: env("DB_URL"),
      port: env("PORT"),
    });
  ...
  it("maps a nested object shape", () => {
    const result = applyMapping({ BASE_URL: "https://example.com", CSRF_SECRET: "abc" }, {
      site: { url: env("BASE_URL") },
      security: { csrf: { secret: env("CSRF_SECRET") } },
    });
    expect(result).toEqual({
      site: { url: "https://example.com" },
      security: { csrf: { secret: "abc" } },
    });
  });

  it("produces undefined for env vars absent from the binding", () => {
    const result = applyMapping({ BASE_URL: "https://example.com" }, {
      site: { url: env("BASE_URL") },
      services: { email: { apiKey: env("EMAIL_API_KEY") } },
    });
  ...
  it("handles a deeply nested map without losing keys", () => {
    const result = applyMapping({ A: "1", B: "2", C: "3" }, {
      x: { y: { z: env("A") } },
      p: env("B"),
      q: env("C"),
    });
  ...
  it("mixes env refs and literal values in nested maps", () => {
    const result = applyMapping({ API_KEY: "secret" }, {
      apiKey: env("API_KEY"),
...
const turnstileGroup = optionalGroup(
  { secretKey: v.string(), siteKey: v.string() },
  { required: "all" },
);
...
describe("optionalGroup — returns group when required fields present", () => {
  ...
  it("applies defaults for missing optional fields", () => {
    const result = v.parse(emailGroup, {
      ...
      from: "from@example.com",
      to: "to@example.com",
    }) as { apiUrl: string; senderName: string } | null;
