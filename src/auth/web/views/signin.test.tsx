...
import type { AuthPasskeyContract } from "./passkey-enrol";
import { SigninView, type SigninViewProps } from "./signin";
...
describe("SigninView across the factor matrix", () => {
  ...
  it("puts the passkey trigger at exactly the emphasis the registry's primary implies, in every legal cell", async () => {
    ...
    expect(rendered).toEqual([
      "email-otp / single => no-trigger / email-field",
      "passkey / single => primary / no-email-field",
      "email-otp+passkey primary=email-otp / single => outline / email-field",
      "email-otp+passkey primary=email-otp / second-factor:always => outline / email-field",
      "email-otp+passkey primary=email-otp / second-factor:when-enrolled => outline / email-field",
      "email-otp+passkey primary=email-otp / second-factor:for-roles => outline / email-field",
      "email-otp+passkey primary=passkey / single => primary / no-email-field",
      "email-otp+passkey primary=passkey / second-factor:always => primary / no-email-field",
      "email-otp+passkey primary=passkey / second-factor:when-enrolled => primary / no-email-field",
      "email-otp+passkey primary=passkey / second-factor:for-roles => primary / no-email-field",
      "email-otp+totp-app / single => no-trigger / email-field",
      "email-otp+totp-app / second-factor:always => no-trigger / email-field",
      "email-otp+totp-app / second-factor:when-enrolled => no-trigger / email-field",
      "email-otp+totp-app / second-factor:for-roles => no-trigger / email-field",
      "passkey+totp-app / single => primary / no-email-field",
      "passkey+totp-app / second-factor:always => primary / no-email-field",
      "passkey+totp-app / second-factor:when-enrolled => primary / no-email-field",
      "passkey+totp-app / second-factor:for-roles => primary / no-email-field",
      "email-otp+passkey+totp-app primary=email-otp / single => outline / email-field",
      "email-otp+passkey+totp-app primary=email-otp / second-factor:always => outline / email-field",
      "email-otp+passkey+totp-app primary=email-otp / second-factor:when-enrolled => outline / email-field",
      "email-otp+passkey+totp-app primary=email-otp / second-factor:for-roles => outline / email-field",
      "email-otp+passkey+totp-app primary=passkey / single => primary / no-email-field",
      "email-otp+passkey+totp-app primary=passkey / second-factor:always => primary / no-email-field",
      "email-otp+passkey+totp-app primary=passkey / second-factor:when-enrolled => primary / no-email-field",
      "email-otp+passkey+totp-app primary=passkey / second-factor:for-roles => primary / no-email-field",
    ]);
  });
  ...
  it("leaves every refused offering with no page to render at all", () => {
    expect(grid.filter((cell) => factorChoices(cell) === null).map((cell) => cell.label)).toEqual([
      ...
      "email-otp / second-factor:always",
      "email-otp / second-factor:when-enrolled",
      "email-otp / second-factor:for-roles",
      "passkey / second-factor:always",
      "passkey / second-factor:when-enrolled",
      "passkey / second-factor:for-roles",
      "totp-app / single",
      "totp-app / second-factor:always",
      "totp-app / second-factor:when-enrolled",
      "totp-app / second-factor:for-roles",
    ]);
  });
