...
See [NAMESPACES.md](docs/NAMESPACES.md) for the authoritative namespace catalog.
...
| `@y-core/forge/warden` | The fleet corpus and `warden sync` (Node/Bun only) | [warden/README.md](warden/README.md) |
| `@y-core/forge/warden/checks` | The documentation, README-export, changelog & design corpus checks | [warden/README.md](warden/README.md) |
| `@y-core/forge/warden/steps` | The gate steps for those checks — `docsStep`, `changelogStep`, … | [warden/README.md](warden/README.md) |
| `@y-core/forge/warden/knowledge` | BM25 retrieval over the corpus — index, search, read, related | [warden/README.md](warden/README.md) |
| `@y-core/forge/warden/mcp` | The MCP server behind `warden serve` — its tools and resources | [warden/README.md](warden/README.md) |
...
Type checking uses `tsc` (`typescript` 7, the native compiler). `validate-exports` verifies, in both directions, that every barrel export resolves at runtime **and** that every `@public`-tagged source symbol is re-exported from its namespace barrel.
